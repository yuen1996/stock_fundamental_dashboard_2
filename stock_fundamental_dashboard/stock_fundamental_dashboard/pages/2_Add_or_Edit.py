...
def comma_number_input(label, value, key, decimals=0):
    ...
    A text-input that:
      • shows commas,
      • keeps a numeric value in st.session_state[key],
      • returns the numeric value.
    """
    txt_key = key + "_txt"

    # init both the display and numeric state once
    ...
    def _fmt():
        raw = str(st.session_state.get(txt_key, "")).replace(",", "")
        try:
            num = float(raw)
    ...
    txt = st.text_input(label, value=st.session_state[txt_key], key=txt_key, on_change=_fmt)

    # also set numeric on every render
    try:
        num = float(str(txt).replace(",", ""))
    except:
        num = 0.0
    st.session_state[key] = num
    return num
...
# === PATCH: single-row Quarter form & save (includes IndustryBucket) ===
def render_quarter_form(stock_name: str, industry: str, industry_bucket: str, QUARTERLY_FIELDS):
    import pandas as pd

    with st.form(f"quarter_form_{stock_name}"):
        st.subheader("➕ Add / Edit — Quarterly row")

        # Basic keys
        q_year = st.number_input(
            "Year", min_value=1900, max_value=2100,
            value=int(st.session_state.get("q_year", 0)) or 0,
            step=1, key=f"q_year_{stock_name}"
        )
        q_quarter = st.text_input(
            "Quarter (Q1–Q4)",
            value=st.session_state.get("q_quarter", "Q1"),
            key=f"q_quarter_{stock_name}"
        )

        # Quarter numeric/text fields (simple inputs; commas OK — parser downstream handles them)
        q_vals = {}
        for label, key in QUARTERLY_FIELDS:
            q_vals[key] = st.text_input(
                label,
                value=st.session_state.get(f"q_{key}_{stock_name}", ""),
                key=f"q_{key}_{stock_name}"
            )

        submitted = st.form_submit_button("💾 Save this quarter")
    ...
    try:
        df = io_helpers.load_data()
        df = io_helpers.ensure_schema(df)

        # Normalize keys
        y = int(q_year) if str(q_year).strip() else None
        q = str(q_quarter or "").strip().upper()
        if q in {"1","2","3","4"}:  # allow "1" -> "Q1"
            q = f"Q{q}"
        if not (y and q in {"Q1","Q2","Q3","Q4"}):
            st.error("Please enter a valid Year and Quarter (Q1–Q4).")
            return

        # Build one fresh row
        new_row = {c: pd.NA for c in io_helpers.ALL_COLUMNS}
        new_row.update({
            "Name": stock_name,
            "Industry": (industry or ""),
            "IndustryBucket": (industry_bucket or ""),
            "Year": y,
            "IsQuarter": True,
            "Quarter": q,
        })
        for _, key in QUARTERLY_FIELDS:
            val = q_vals.get(key, "")
            new_row[key] = val if val != "" else pd.NA

        # Upsert this (Name, Year, Quarter, IsQuarter=True)
        mask = (
            df["Name"].astype(str).str.lower().eq(str(stock_name).lower()) &
            df["IsQuarter"].eq(True) &
            df["Year"].astype("Int64").eq(y) &
            df["Quarter"].astype(str).eq(q)
        )
        df = df.loc[~mask]
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)

        # Optionally sync bucket/text across all rows for this stock
        if industry_bucket:
        ...
        st.success(f"Saved quarterly row: {stock_name} · {y} {q}")
...
if st.button("💾 Save All Annual Changes"):
    ...
    with tabs_top[1]:
        ...
        # ✅ New single-row form (Enter key safe, reliable upsert)
        render_quarter_form(stock_name, industry, industry_bucket, QUARTERLY_FIELDS)
