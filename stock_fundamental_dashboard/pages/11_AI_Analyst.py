...
def _cagr_from_series(years: List[int], values: List[float], min_year_span: int = 2) -> float | None:
    """Compute CAGR using oldest vs newest non-null values; requires at least min_year_span distinct years."""
    try:
        pairs = [(int(y), float(v)) for y, v in zip(years, values) if v is not None and not (isinstance(v, float) and math.isnan(v))]
        ...
        end_y, end_v = pairs[-1]
        span = end_y - start_y
        if span < min_year_span or start_v is None or end_v is None or start_v <= 0 or end_v <= 0:
            return None
        return (end_v / start_v) ** (1.0 / span) - 1.0
    except Exception:
        return None
...
def compute_momentum_bits(name: str) -> dict | None:
    ...
    try:
        ...
        ret_3m = _ret_back(63)   # ~63 trading days
        ret_6m = _ret_back(126)  # ~126
        ret_12m = _ret_back(252) # ~252
...
def collect_bundle(name: str, full: bool = False) -> dict[str, Any]:
    ...
    try:
        # supply current price if we have it so PE/PS/PB can be derived internally when supported
        curr_price = _safe_float((ohlc or {}).get("price")) or _safe_float(latest_annual_dict.get("CurrentPrice")) or _safe_float(latest_annual_dict.get("SharePrice"))
    ...
    # Derive extra TTM ratios if inputs exist
    try:
    ...
    if not annual.empty:
        try:
            ...
            rev = pd.to_numeric(ax.get("Revenue"), errors="coerce").replace([np.inf, -np.inf], np.nan)
            eps = pd.to_numeric(ax.get("EPS"), errors="coerce").replace([np.inf, -np.inf], np.nan)
            npf = pd.to_numeric(ax.get("NetProfit"), errors="coerce").replace([np.inf, -np.inf], np.nan)
            roe = pd.to_numeric(ax.get("ROE (%)"), errors="coerce").replace([np.inf, -np.inf], np.nan)

            years = ax["Year"].tolist()
            growth["Revenue CAGR (5y)"] = _cagr_from_series(years, rev.tolist(), min_year_span=4) if "Revenue" in ax.columns else None
            growth["Revenue CAGR (3y)"] = _cagr_from_series(years[-4:], rev.tolist()[-4:], min_year_span=2) if "Revenue" in ax.columns else None
            if "EPS" in ax.columns:
                growth["EPS CAGR (5y)"] = _cagr_from_series(years, eps.tolist(), min_year_span=4)
                growth["EPS CAGR (3y)"] = _cagr_from_series(years[-4:], eps.tolist()[-4:], min_year_span=2)
            # ROE stability (median & stdev over 5y)
            if "ROE (%)" in ax.columns:
                roe5 = roe.dropna().tail(5)
    ...
    # ---- Assemble bundle ----
    # include more TTM pieces (CFO/CapEx/FCF/Health)
    ttm_keep = [
        ...
        "Interest Coverage (EBITDA/Fin)","MarketCap","Current Price","Net Cash (Debt)","Net Cash / MC (%)",
        "DataHealth"
    ]
    bundle: Dict[str, Any] = {
        "name": name,
...
# Default question used only for the very first message
...
if not st.session_state["chat_inited"]:
    # seed input text for mention detection preview
    seed_text = default_q
else:
    seed_text = ""

# Build context for preview (mentions from last user message if present)
...
def build_prompt_messages(question: str, catalog: list[dict], packs: list[dict]) -> List[Dict[str,str]]:
    ...
    try:
        ...
        # very defensive: replace NaN/inf
        def _sanitize(o):
...
# On first render, optionally suggest a starting message (but do not auto-send)
if not st.session_state["chat_inited"]:
...
if prompt is not None:
    if not st.session_state.get("ai_api_key"):
        ...
        # Build mention list from this prompt
