...
def collect_bundle(name: str, full: bool = False) -> dict[str, Any]:
    ...
    # Ensure ROE (%) exists; compute if missing/empty
    if ("ROE (%)" not in annual.columns) or annual["ROE (%)"].isna().all():
        _np  = pd.to_numeric(annual.get("NetProfit"), errors="coerce")
        _eq  = pd.to_numeric(annual.get("ShareholderEquity"), errors="coerce")
        annual["ROE (%)"] = (_np / _eq * 100.0).replace([np.inf, -np.inf], np.nan)
