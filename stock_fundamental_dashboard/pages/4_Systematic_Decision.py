...
def _detail_df(detail_dict):
    items = list(_flat_pairs(detail_dict or {}))
    return pd.DataFrame(items, columns=["Metric", "Value"]) if items else pd.DataFrame(columns=["Metric", "Value"])
...
st.markdown("""
<style>
/* Zebra rows for all dataframes/editors */
div[data-testid="stDataFrame"] table tbody tr:nth-child(odd) td,
div[data-testid="stDataEditor"] table tbody tr:nth-child(odd) td{ background:#fbfdff !important; }
div[data-testid="stDataFrame"] table tbody tr:nth-child(even) td,
div[data-testid="stDataEditor"] table tbody tr:nth-child(even) td{ background:#ffffff !important; }

/* Make expanders in the detail panel colorful and easier to scan */
.detail-wrap [data-testid="stExpander"] .streamlit-expanderHeader{
  background:#f8fafc; border-left:6px solid var(--info); border-radius:10px;
}
.detail-wrap [data-testid="stExpander"]:nth-of-type(2n) .streamlit-expanderHeader{
  background:#fff7ed; border-left-color: var(--warning);
}
.detail-wrap [data-testid="stExpander"]:nth-of-type(3n) .streamlit-expanderHeader{
  background:#f0fdf4; border-left-color: var(--success);
}
.detail-wrap [data-testid="stExpander"]:nth-of-type(4n) .streamlit-expanderHeader{
...
# NEW: master toggle to show/hide the whole details panel
show_details_panel = st.toggle("Show details panel", value=False)

if not show_details_panel:
    st.caption("Turn on the switch above to inspect per-block calculations.")
elif dec_df.empty:
    ...
    if show_calc:
        ...
        # Annual ratios snapshot (what you asked to review vs prior FY)
        with st.expander("Annual ratios snapshot (last 5 FY; compares latest vs previous)", expanded=False):
            ...
            # Pick any of these columns that actually exist in your data
            prefer_cols = [
            ...
            if cols:
                st.dataframe(ann5[cols], use_container_width=True, height=220)
            else:
        ...
        # Alerts (spikes, margin drops, debt/payout jumps, coverage low, dilution)
        ...
        # Optional: download the full evaluation JSON for audit
        ...
        # Compact view: just the key trend (NPM) and any alerts
        ann = stock_rows[stock_rows["IsQuarter"] != True].copy().sort_values("Year").tail(5)
        npm_tbl = pd.DataFrame({"Year": ann["Year"]})
        with np.errstate(divide="ignore", invalid="ignore"):
            npm = pd.to_numeric(ann.get("NetProfit"), errors="coerce") / pd.to_numeric(ann.get("Revenue"), errors="coerce")
