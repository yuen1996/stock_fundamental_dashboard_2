...
# ──────────────────────────────────────────────────────────
# Score calculation details — pick PASS / FAIL / All
# ──────────────────────────────────────────────────────────
st.markdown(
    '<div class="sec info"><div class="t">🔍 Score calculation details</div>'
    '<div class="d">Pick any stock (PASS or FAIL) and inspect every block and input the funnel used</div></div>',
    unsafe_allow_html=True
)
...
if dec_df.empty:
    ...
    with col_f:
        src_choice = st.radio("Show", ["PASS", "FAIL", "All"], index=0, horizontal=True)
    ...
    if not names:
        ...
        if show_calc2:
            # helper to flatten nicely with commas (reuse your helpers)
            def _flat_pairs(d, prefix=""):
