            ...
            if fetch:
                if not _YF_OK:
                    ...
                    if df_y is not None and not df_y.empty:
                        ...
                        # ✅ Save a clean OHLCV file (no preview-only columns)
                        keep = ["Date", "Open", "High", "Low", "Close"]
                        if "Volume" in df_y.columns:
                            keep.append("Volume")
                        df_save = df_y[keep].copy()

                        path = _save_csv_for(name, df_save)
                        st.success(f"Saved {len(df_save):,} rows to **{path}**")

                        # ⬇️ Offer a direct download of what we just saved
                        st.download_button(
                            "⬇️ Download fetched CSV",
                            data=df_save.to_csv(index=False).encode("utf-8"),
                            file_name=f"{_safe_name(name)}.csv",
                            mime="text/csv",
                            use_container_width=False,
                        )

                        # reload the page so top-3 tables show the updated file (with Volume)
                        try:
                            st.rerun()
                        except Exception:
