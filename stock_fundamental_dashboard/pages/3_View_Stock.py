    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("📥 Download — Annual Raw (JSON)", expanded=False):
                # (only used for the on-screen preview; export below is simplified)
                try:
                    vis = disp_num.reset_index() if isinstance(disp_num, pd.DataFrame) else pd.DataFrame()
                except Exception:
                    vis = pd.DataFrame()

                # Keep matrix in same visible column order (includes any "TTM ..." at the end)
                ...
                # Show an inline preview of the TTM components (readable, numbers only)
                if raw_ttm_detail:
                ...
                # ---- SIMPLE, human-readable annual JSON (flat per-year records)
                # keep only true year columns (skip any "TTM ..." column)
                ...
                for y in years_in_matrix:
                    try:
                        ...
                        continue
                    rec = {"year": y_int}
                    # add every defined field we have in the matrix (Revenue, Net Profit, Share Price, etc.)
                    for sec, items in ANNUAL_SECTIONS:
                ...
                # Compact TTM section (what + what)
                ttm_simple = {
                    "label": (raw_ttm_detail or {}).get("label"),
                    "quarters_used": (raw_ttm_detail or {}).get("quarters_used", []),
                    "components": (raw_ttm_detail or {}).get("components", {}),
                }
                ...
                ann_payload = {
                    ...
                    "annual": annual_simple,   # [{year: 2020, Revenue: ..., Net Profit: ..., Share Price: ...}, ...]
                    "ttm": ttm_simple          # {"label": "...", "quarters_used": [...], "components": {...}}
                }
