...
def _dep_total_from_row(row):
    """Sum splits if present, else fall back to a single DepAmort-style field."""
    parts = [_num(row.get("DepPPE")), _num(row.get("DepInvProp")), _num(row.get("DepROU"))]
    parts = [p for p in parts if np.isfinite(p)]
    if parts:
        return float(np.nansum(parts))
    # fallback names that sometimes appear in older years
    da = _num(row.get("DepAmort")) or _num(row.get("Depreciation & Amortization"))
    return da if np.isfinite(da) else np.nan

def derive_ebitda_from_row(row):
    """
    Robust EBITDA:
      1) raw EBITDA if present;
      2) EBIT + D&A, where EBIT = OperatingProfit or (GrossProfit - Admin - SellDist);
      3) NetProfit + FinanceCosts + IncomeTax + D&A
    """
    raw = _num(row.get("EBITDA"))
    if np.isfinite(raw) and raw != 0:
        return raw

    # EBIT
    op = _num(row.get("OperatingProfit"))
    if not np.isfinite(op):
        gp  = _num(row.get("GrossProfit"))
        adm = _num(row.get("AdminExpenses"))
        sde = _num(row.get("SellDistExpenses"))
        if all(np.isfinite(v) for v in (gp, adm, sde)):
            op = gp - adm - sde

    da = _dep_total_from_row(row)
    if np.isfinite(op) and np.isfinite(da):
        return op + da

    # NP + Fin + Tax + D&A (last resort)
    npf = _num(row.get("NetProfit"))
    fin = _num(row.get("FinanceCosts"))
    tax = _num(row.get("IncomeTax"))
    if not np.isfinite(tax):
        # a few alternate headings we sometimes see
        for alt in ("Tax", "TaxExpense", "Income Tax Expense"):
            t = _num(row.get(alt))
            if np.isfinite(t):
                tax = t
                break
    ...
    if any(np.isfinite(v) for v in vals):
        return float(np.nansum([v for v in vals if np.isfinite(v)]))
...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # ---------- Show calculation details — TTM  (REPLACE THIS WHOLE BLOCK) ----------
            with st.expander("Show calculation details — TTM", expanded=False):

                # Small helpers (safe parsing + safe formatting)
                def _n(x):
                    import numpy as np
                    import pandas as pd
                    try:
                ...
                def _last4(col):
                    import pandas as pd, numpy as np
                    s = pd.to_numeric(quarterly.get(col, pd.Series(dtype="float64")), errors="coerce").dropna()
                    return s.iloc[-4:] if len(s) >= 1 else s

                def _sum_last4(col):
                    import numpy as np
                    s4 = _last4(col)
                    return float(s4.sum()) if not s4.empty else np.nan
                ...
                # --- Context table: last 6–8 quarters so user can see the window you sum
                try:
                    q_show = quarterly.copy()
                    # Keep only the columns we reference below to keep the table compact
                    cols_needed = [
                        "Year", "Quarter",
                        "Q_Revenue", "Q_GrossProfit", "Q_NetProfit",
                        "Q_NumShares", "Q_EndQuarterPrice", "Q_ShareholderEquity",
                        "Q_CFO", "Q_CapEx", "Q_IncomeTax",
                        "Q_DepPPE", "Q_DepInvProp", "Q_DepROU",
                        "Q_Cash", "Q_LeaseLiabCurrent", "Q_LeaseLiabNonCurrent",
                        "Q_Borrowings", "Q_OtherLoans"
                    ]
                    cols_present = [c for c in cols_needed if c in q_show.columns]
                    q_show = q_show[cols_present].tail(8)
                    st.dataframe(q_show, use_container_width=True)
                except Exception:
                    pass

                # ---- Base TTM sums (same as your existing ones)
                rev_ttm   = _sum_last4("Q_Revenue")
                gp_ttm    = _sum_last4("Q_GrossProfit")
                np_ttm    = _sum_last4("Q_NetProfit")

                # Average shares over the TTM window; fallback to last quarter or latest annual
                import numpy as np, pandas as pd
                shares_4  = _last4("Q_NumShares")
                shares_av = float(shares_4.mean()) if not shares_4.empty else _n(annual.get("NumShares", pd.Series(dtype="float64")).dropna().iloc[-1] if "NumShares" in annual.columns and not annual["NumShares"].dropna().empty else np.nan)

                eps_ttm   = (np_ttm / shares_av) if (np.isfinite(np_ttm) and np.isfinite(shares_av) and shares_av != 0) else np.nan

                # Price for TTM: prefer last Q_EndQuarterPrice, then price_now if present, then last annual SharePrice
                price_ttm = _last_val("Q_EndQuarterPrice")
                if not np.isfinite(price_ttm):
                    price_ttm = _n(locals().get("price_now", np.nan))
                if not np.isfinite(price_ttm):
                    try:
                        price_ttm = _n(annual.get("SharePrice").dropna().iloc[-1]) if "SharePrice" in annual.columns else np.nan
                    except Exception:
                        price_ttm = np.nan

                # Market cap for TTM
                shares_last_q = _last_val("Q_NumShares")
                if not np.isfinite(shares_last_q):
                    shares_last_q = shares_av
                mc_ttm = (price_ttm * shares_last_q) if (np.isfinite(price_ttm) and np.isfinite(shares_last_q)) else np.nan

                # Equity for P/B (prefer last Q, else last annual)
                eq_ttm = _last_val("Q_ShareholderEquity")
                if not np.isfinite(eq_ttm):
                    try:
                        eq_ttm = _n(annual.get("ShareholderEquity").dropna().iloc[-1]) if "ShareholderEquity" in annual.columns else np.nan
                    except Exception:
                        eq_ttm = np.nan

                # Finance costs (for EBITDA/coverage) — quarterly sum
                fin_ttm  = _sum_last4("Q_FinanceCosts")

                # Taxes/D&A (quarterly sums)
                tax_ttm     = _sum_last4("Q_IncomeTax")
                depppe_ttm  = _sum_last4("Q_DepPPE")
                depinv_ttm  = _sum_last4("Q_DepInvProp")
                deprou_ttm  = _sum_last4("Q_DepROU")
                da_ttm      = float(np.nansum([depppe_ttm, depinv_ttm, deprou_ttm]))

                # EBITDA (TTM): NP + Fin + Tax + D&A (robust to missing pieces)
                ebitda_ttm = float(np.nansum([np_ttm, fin_ttm, tax_ttm, da_ttm]))

                # Net debt snapshot for EV/EBITDA → latest quarter snapshot
                cash_last = _last_val("Q_Cash")
                debt_last = float(np.nansum([
                    _last_val("Q_Borrowings"),
                    _last_val("Q_OtherLoans"),
                    _last_val("Q_LeaseLiabCurrent"),
                    _last_val("Q_LeaseLiabNonCurrent"),
                ]))
                net_debt  = (debt_last - cash_last) if (np.isfinite(debt_last) and np.isfinite(cash_last)) else np.nan

                # Ratios you already show
                pe_ttm  = (price_ttm / eps_ttm) if (np.isfinite(price_ttm) and np.isfinite(eps_ttm) and eps_ttm != 0) else np.nan
                ps_ttm  = (mc_ttm / rev_ttm)    if (np.isfinite(mc_ttm)   and np.isfinite(rev_ttm) and rev_ttm != 0) else np.nan
                pb_ttm  = (mc_ttm / eq_ttm)     if (np.isfinite(mc_ttm)   and np.isfinite(eq_ttm)  and eq_ttm  != 0) else np.nan
                ev_ebit = ((mc_ttm + net_debt) / ebitda_ttm) if (np.isfinite(mc_ttm) and np.isfinite(net_debt) and np.isfinite(ebitda_ttm) and ebitda_ttm != 0) else np.nan

                # ------- NEW: the extra TTM lines you asked for -------
                cfo_ttm        = _sum_last4("Q_CFO")
                capex_ttm      = _sum_last4("Q_CapEx")
                cash_ttm       = _last_val("Q_Cash")
                lease_cur_ttm  = _last_val("Q_LeaseLiabCurrent")
                lease_ncur_ttm = _last_val("Q_LeaseLiabNonCurrent")
                borrow_ttm     = _last_val("Q_Borrowings")
                otherloan_ttm  = _last_val("Q_OtherLoans")

                # ---------- Render bullets ----------
                st.markdown("**TTM window:** latest four quarters → to " + _fmt(_last_val("Q_Revenue"), 0).replace("–", "latest"))

                # Your existing headline TTM bullets (kept)
                st.markdown(
                    f"- **TTM Revenue** = sum(last 4 Q_Revenue) = {_fmt(rev_ttm, 0)}  \n"
                    f"- **TTM Gross Margin** = GrossProfit_TTM ÷ Revenue_TTM = {_fmt(gp_ttm, 0)} ÷ {_fmt(rev_ttm, 0)} = **{_fmt((gp_ttm / rev_ttm * 100.0) if (np.isfinite(gp_ttm) and np.isfinite(rev_ttm) and rev_ttm != 0) else np.nan, 2)}%**  \n"
                    f"- **TTM Net Profit** = sum(last 4 Q_NetProfit) = {_fmt(np_ttm, 0)}  \n"
                    f"- **TTM EPS** = NetProfit_TTM ÷ avg(shares) = {_fmt(np_ttm, 0)} ÷ {_fmt(shares_av, 0)} = **{_fmt(eps_ttm, 4)}**  \n"
                    f"- **P/E (TTM)** = Price ÷ EPS_TTM = {_fmt(price_ttm, 4)} ÷ {_fmt(eps_ttm, 4)} = **{_fmt(pe_ttm, 2)}**  \n"
                    f"- **P/S (TTM)** = MarketCap ÷ Revenue_TTM = {_fmt(mc_ttm, 0)} ÷ {_fmt(rev_ttm, 0)} = **{_fmt(ps_ttm, 2)}**  \n"
                    f"- **P/B (TTM)** = MarketCap ÷ Equity(latest) = {_fmt(mc_ttm, 0)} ÷ {_fmt(eq_ttm, 0)} = **{_fmt(pb_ttm, 2)}**  \n"
                    f"- **EV/EBITDA (TTM)** = (MarketCap + NetDebt) ÷ EBITDA_TTM = ({_fmt(mc_ttm, 0)} + {_fmt(net_debt, 0)}) ÷ {_fmt(ebitda_ttm, 0)} = **{_fmt(ev_ebit, 2)}**  \n"
                )

                # New cash-flow / balance-sheet TTM bullets (from your orange cells)
                st.markdown(
                    f"- **Cash and bank balance (TTM)** = latest Q_Cash = **{_fmt(cash_ttm, 0)}**  \n"
                    f"- **Current lease liabilities (TTM)** = latest Q_LeaseLiabCurrent = **{_fmt(lease_cur_ttm, 0)}**  \n"
                    f"- **Non-current lease liabilities (TTM)** = latest Q_LeaseLiabNonCurrent = **{_fmt(lease_ncur_ttm, 0)}**  \n"
                    f"- **Net cash flow from operating activities (TTM)** = sum(last 4 Q_CFO) = **{_fmt(cfo_ttm, 0)}**  \n"
                    f"- **Purchase of property, plant & equipment (TTM)** = sum(last 4 Q_CapEx) = **{_fmt(capex_ttm, 0)}**  \n"
                    f"- **Income Tax Expense (TTM)** = sum(last 4 Q_IncomeTax) = **{_fmt(tax_ttm, 0)}**  \n"
                    f"- **Depreciation of PPE (TTM)** = sum(last 4 Q_DepPPE) = **{_fmt(depppe_ttm, 0)}**  \n"
                    f"- **Depreciation of investment property (TTM)** = sum(last 4 Q_DepInvProp) = **{_fmt(depinv_ttm, 0)}**  \n"
                    f"- **Depreciation of right-of-use assets (TTM)** = sum(last 4 Q_DepROU) = **{_fmt(deprou_ttm, 0)}**  \n"
                    f"- **Borrowings (TTM)** = latest Q_Borrowings = **{_fmt(borrow_ttm, 0)}**  \n"
                    f"- **Other loans (TTM)** = latest Q_OtherLoans = **{_fmt(otherloan_ttm, 0)}**  \n"
                )

                # ---------- JSON payload (include all ANNUAL rows + extra TTM items) ----------
                # Build a full-year-by-year export using your ANNUAL_SECTIONS label mapping.
                annual_rows = []
                try:
                    ann = annual.copy()
                    ann["Year"] = pd.to_numeric(ann.get("Year"), errors="coerce")
                    ann = ann.dropna(subset=["Year"]).sort_values("Year")

                    # Flatten the (label, key) pairs we want to export with friendly labels
                    annual_items = []
                    for _sec, _items in ANNUAL_SECTIONS:
                        annual_items.extend(_items)  # (label, key)

                    for _, r in ann.iterrows():
                        y = int(float(r["Year"]))
                        rec = {"year": y}
                        for label, key in annual_items:
                            if key in r:
                                rec[label] = _n(r[key])  # keep numbers as floats (None if NaN)
                        annual_rows.append(rec)
                except Exception:
                    # Fail-safe: empty list if anything odd happens
                    annual_rows = []

                # Work out which specific quarters the TTM window used (last 4)
                quarters_used = []
                try:
                    q4_for_json = quarterly.copy()
                    q4_for_json["Year"] = pd.to_numeric(q4_for_json.get("Year"), errors="coerce")
                    q4_for_json = q4_for_json.dropna(subset=["Year"])
                    # Preserve the natural order -> take the last 4 rows as your TTM window
                    q4_for_json = q4_for_json.sort_values(["Year", "Quarter"]).tail(4)
                    for _, row_q in q4_for_json.iterrows():
                        y = int(float(row_q["Year"])) if "Year" in row_q and pd.notna(row_q["Year"]) else None
                        qu = str(row_q["Quarter"]) if "Quarter" in row_q and pd.notna(row_q["Quarter"]) else ""
                        if y is not None:
                            quarters_used.append([y, qu])
                except Exception:
                    pass

                # Assemble the final JSON payload (annual + ttm)
                payload = {
                    "stock": stock_name,
                    "industry": industry,
                    ...
                    "annual": annual_rows,
                    "ttm": {
                        "label": f"TTM (to {int(q4_for_json['Year'].iloc[-1])})" if quarters_used else "TTM",
                        "quarters_used": quarters_used,
                        "components": {
                            # Headline TTM sums/snapshots
                            "TTM Revenue": rev_ttm,
                            "TTM Net Profit": np_ttm,
                            "TTM Gross Profit": gp_ttm,
                            "TTM Cost Of Sales": (rev_ttm - gp_ttm) if (np.isfinite(rev_ttm) and np.isfinite(gp_ttm)) else np.nan,
                            "CFO (TTM)": cfo_ttm,
                            "CapEx (TTM)": capex_ttm,
                            "Income Tax (TTM)": tax_ttm,
                            "Depreciation PPE (TTM)": depppe_ttm,
                            "Depreciation of investment property (TTM)": depinv_ttm,
                            "Depreciation of right-of-use assets (TTM)": deprou_ttm,
                            "EBITDA (TTM)": ebitda_ttm,
                            # Snapshots / latests
                            "Shares (latest)": shares_last_q,
                            "Current Asset (latest)": _last_val("Q_CurrentAsset"),
                            "Current Liability (latest)": _last_val("Q_CurrentLiability"),
                            "Equity (latest)": eq_ttm,
                            "Cash (latest)": cash_last,
                            "Current lease liabilities (latest)": lease_cur_ttm,
                            "Non-current lease liabilities (latest)": lease_ncur_ttm,
                            "Borrowings (latest)": borrow_ttm,
                            "Other loans (latest)": otherloan_ttm,
                            "Price (latest)": price_ttm,
                        }
                    }
                }

                _download_json_button(
                    "📥 Download Annual Raw + TTM (JSON)",
                    payload,
                    f"{stock_name}_annual_raw.json",
                    key=f"annual_raw_full_{stock_name}"
                )
            ...
            # ---- defaults so downstream expanders don't crash if TTM isn't available
            Rev_q = GP_q = OP_q = NP_q = EPS_q = SH_q = PX_q = EBITDA_q = pd.Series(dtype="float64")

            rev_ttm = gp_ttm = op_ttm = np_ttm = ebitda_ttm = eps_ttm = None
            gm_ttm = nm_ttm = None
            price_now = market_cap = equity_latest = pb_ttm = ev_ebitda = None
            debt_latest = cash_latest = None
            ...
            # --- Latest FY row from `annual`
            try:
                _yr_series = pd.to_numeric(annual.get("Year", pd.Series(index=annual.index)), errors="coerce").dropna()
                _latest_year = int(_yr_series.max()) if not _yr_series.empty else None
            except Exception:
                _latest_year = None

            _raw = {}
            if _latest_year is not None:
                _raw_rec = annual[pd.to_numeric(annual.get("Year"), errors="coerce") == _latest_year].tail(1).to_dict("records")
                _raw = _raw_rec[0] if _raw_rec else {}

            def first_from_raw(*keys):
                for k in keys:
                    if k in _raw:
                        v = n(_raw.get(k))
                        if np.isfinite(v):
            ...
            # --- Inputs (prefer FY raw; sensible fallbacks)
            cfo    = first_from_raw("CFO", "OperatingCashFlow", "Operating Cash Flow",
                                    "Net Cash From Operating Activities", "NetCashFromOperations")
            capex  = first_from_raw("CapEx", "CapitalExpenditure", "Capital Expenditures",
                                    "PurchasePPE", "Purchase of PPE", "Purchase of property, plant and equipment")
            rev    = first_from_raw("Revenue")
            npf    = first_from_raw("NetProfit", "Net Profit")
            shares = first_from_raw("NumShares", "Number of Shares", "Number of shares", "ShareOutstanding", "Shares")

            # Price: prefer FY share price; fallback to live price_now if available
            price_fy       = first_from_raw("SharePrice", "Current Share Price", "End of year share price", "Each end of year share price")
            price_now_safe = n(locals().get("price_now", np.nan))
            price_used     = price_fy if np.isfinite(price_fy) else price_now_safe

            # Market cap estimate
            market_cap_est = (price_used * shares) if (np.isfinite(price_used) and np.isfinite(shares)) else np.nan

            # Latest cash & debt: prefer last quarterly snapshot; fallback to FY values
            def _last_q(col):
                try:
                    s = pd.to_numeric(quarterly.get(col), errors="coerce").dropna()
                    return float(s.iloc[-1]) if not s.empty else np.nan
                except Exception:
                    return np.nan

            cash_latest_val = _last_q("Q_Cash")
            if not np.isfinite(cash_latest_val):
                cash_latest_val = first_from_raw("Cash", "CashAndCashEquivalents", "Cash & Cash Equivalents")

            debt_latest_val = 0.0
            parts = [
                ("Q_Borrowings",              "Borrowings"),
                ("Q_OtherLoans",              "OtherLoans"),
                ("Q_LeaseLiabCurrent",        "LeaseLiabCurrent"),
                ("Q_LeaseLiabNonCurrent",     "LeaseLiabNonCurrent"),
            ]
            for qcol, acol in parts:
                vq = _last_q(qcol)
                if np.isfinite(vq):
                    debt_latest_val += vq
                else:
                    va = first_from_raw(acol)
                    if np.isfinite(va):
                        debt_latest_val += va
            if debt_latest_val == 0.0 and not any(np.isfinite(_last_q(q)) for q, _ in parts):
                debt_latest_val = np.nan

            # EBITDA (FY): reported → EBIT + D&A → NP + Fin + Tax + D&A
            ebitda_fy = first_from_raw("EBITDA")
            try:
                da_fy = _dep_total_from_row(_raw)  # your helper if present
            except Exception:
                da_fy = first_from_raw("DepAmort", "Depreciation & Amortization", "DepreciationAmortization")

            op = first_from_raw("OperatingProfit", "Operating Profit")
            if not np.isfinite(op):
                gp_  = first_from_raw("GrossProfit", "Gross Profit")
                adm_ = first_from_raw("AdminExpenses", "Administrative Expenses")
                sde_ = first_from_raw("SellDistExpenses", "Selling & Distribution Expenses")
                if all(np.isfinite(x) for x in (gp_, adm_, sde_)):
                    op = gp_ - adm_ - sde_

            if not np.isfinite(ebitda_fy):
                if np.isfinite(op) and np.isfinite(da_fy):
                    ebitda_fy = op + da_fy
                else:
                    fin_ = first_from_raw("FinanceCosts", "Finance Costs", "Finance cost")
                    tax_ = first_from_raw("IncomeTax", "Tax", "TaxExpense", "Income Tax Expense")
                    candidates = [npf, fin_, tax_, da_fy]
                    if any(np.isfinite(x) for x in candidates):
                        ebitda_fy = float(np.nansum([x for x in candidates if np.isfinite(x)]))
                    else:
                        ebitda_fy = np.nan

            fincost = first_from_raw("FinanceCosts", "Finance Costs", "Finance cost")

            # --- Calculations (FY)
            fcf         = (cfo - capex) if (np.isfinite(cfo) and np.isfinite(capex)) else np.nan
            cash_conv   = (cfo / npf * 100.0) if (np.isfinite(cfo) and np.isfinite(npf) and npf != 0) else np.nan
            fcf_ps      = (fcf / shares) if (np.isfinite(fcf) and np.isfinite(shares) and shares != 0) else np.nan
            fcf_margin  = (fcf / rev * 100.0) if (np.isfinite(fcf) and np.isfinite(rev) and rev != 0) else np.nan
            fcf_yield   = (fcf / market_cap_est * 100.0) if (np.isfinite(fcf) and np.isfinite(market_cap_est) and market_cap_est != 0) else np.nan
            net_cash    = (cash_latest_val - debt_latest_val) if (np.isfinite(cash_latest_val) and np.isfinite(debt_latest_val)) else np.nan
            netcash_mc  = (net_cash / market_cap_est * 100.0) if (np.isfinite(net_cash) and np.isfinite(market_cap_est) and market_cap_est != 0) else np.nan
            icov        = (ebitda_fy / fincost) if (np.isfinite(ebitda_fy) and np.isfinite(fincost) and fincost != 0) else np.nan
            debt_fcf    = (debt_latest_val / fcf) if (np.isfinite(debt_latest_val) and np.isfinite(fcf) and fcf > 0) else np.nan

            _label_yr = f"FY{_latest_year}" if _latest_year else "Latest FY"

            # ---- UI tiles ----
            r1c1, r1c2, r1c3, r1c4 = st.columns(4)
            with r1c1:
                st.metric(f"{_label_yr} CFO", _fmt2(cfo, 0))
                st.metric("Cash Conversion (CFO/NP)", f"{_fmt2(cash_conv, 2)}%")
            with r1c2:
                st.metric(f"{_label_yr} CapEx", _fmt2(capex, 0))
                st.metric("Debt / FCF (yrs)", _fmt2(debt_fcf, 2))
            with r1c3:
                st.metric(f"{_label_yr} FCF", _fmt2(fcf, 0))
                st.metric(f"FCF / Share ({_label_yr})", _fmt2(fcf_ps, 4))
            with r1c4:
                st.metric(f"FCF Margin ({_label_yr})", f"{_fmt2(fcf_margin, 2)}%")
                st.metric(f"FCF Yield ({_label_yr})", f"{_fmt2(fcf_yield, 2)}%")

            r2c1, r2c2, r2c3, r2c4 = st.columns(4)
            with r2c1:
                st.metric("Net Cash (Debt)", _fmt2(net_cash, 0))
            with r2c2:
                st.metric("Net Cash / MC", f"{_fmt2(netcash_mc, 2)}%")
            with r2c3:
                st.metric("Interest Coverage", _fmt2(icov, 2))
            with r2c4:
                st.metric("Market Cap (est.)", _fmt2(market_cap_est, 0))

            # ---- Details (FY formulas with numbers)
            with st.expander(f"Show calculation details — Cash Flow Wealth ({_label_yr})", expanded=False):
                st.markdown(f"**{_label_yr}**")
                st.markdown(
                    f"- **CFO** (FY) = **{(cfo if np.isfinite(cfo) else np.nan):,.0f}**  \n"
                    f"- **CapEx** (FY) = **{(capex if np.isfinite(capex) else np.nan):,.0f}**  \n"
                    ...
                    f"- **FCF / Share** ({_label_yr}) = FCF ÷ Shares = {(fcf if np.isfinite(fcf) else np.nan):,.0f} ÷ {(shares if np.isfinite(shares) else np.nan):,.0f} = **{(fcf_ps if np.isfinite(fcf_ps) else np.nan):,.4f}**  \n"
                    f"- **FCF Margin** ({_label_yr}) = FCF ÷ Revenue = {(fcf if np.isfinite(fcf) else np.nan):,.0f} ÷ {(rev if np.isfinite(rev) else np.nan):,.0f} = **{(fcf_margin if np.isfinite(fcf_margin) else np.nan):,.2f}%**  \n"
                    f"- **FCF Yield** ({_label_yr}) = FCF ÷ Market Cap = {(fcf if np.isfinite(fcf) else np.nan):,.0f} ÷ {(market_cap_est if np.isfinite(market_cap_est) else np.nan):,.0f} = **{(fcf_yield if np.isfinite(fcf_yield) else np.nan):,.2f}%**  \n"
                    f"- **Net Cash (Debt)** = Cash − Debt = {(cash_latest_val if np.isfinite(cash_latest_val) else np.nan):,.0f} − {(debt_latest_val if np.isfinite(debt_latest_val) else np.nan):,.0f} = **{(net_cash if np.isfinite(net_cash) else np.nan):,.0f}**  \n"
                    f"- **Net Cash / MC** = Net Cash ÷ Market Cap = {(net_cash if np.isfinite(net_cash) else np.nan):,.0f} ÷ {(market_cap_est if np.isfinite(market_cap_est) else np.nan):,.0f} = **{(netcash_mc if np.isfinite(netcash_mc) else np.nan):,.2f}%**  \n"
                    f"- **Interest Coverage** = EBITDA ÷ Finance Costs = {(ebitda_fy if np.isfinite(ebitda_fy) else np.nan):,.0f} ÷ {(fincost if np.isfinite(fincost) else np.nan):,.0f} = **{(icov if np.isfinite(icov) else np.nan):,.2f}**"
                )

                # ↓↓↓ place the download button INSIDE the expander ↓↓↓
                cfw_payload = {
                    "stock": stock_name,
                    "label": _label_yr,
                    "inputs": {
                        "cfo": cfo, "capex": capex, "revenue": rev, "net_profit": npf,
                        "shares": shares, "price": price_used, "market_cap_est": market_cap_est,
                        "cash_latest": cash_latest_val, "debt_latest": debt_latest_val,
                        "ebitda_fy": ebitda_fy, "finance_costs": fincost
                    },
                    "metrics": {
                        "fcf": fcf, "fcf_per_share": fcf_ps, "fcf_margin_pct": fcf_margin,
                        "fcf_yield_pct": fcf_yield, "cash_conversion_pct": cash_conv,
                        "net_cash": net_cash, "net_cash_mc_pct": netcash_mc,
                        "interest_coverage": icov, "debt_to_fcf_years": debt_fcf
                    }
                }
                _download_json_button(
                    "📥 Download Cash-Flow Wealth (FY) JSON",
                    cfw_payload,
                    f"{stock_name}_cash_flow_wealth_fy.json",
                    key=f"cfw_fy_{stock_name}"
                )
            ...
            for _, row in annual.iterrows():
                rowx = row.copy()
                # force valuation metrics to use today's price (P/E, P/B, DY, EV/EBITDA, etc.)
                try:
                ...
                # --- ADD: surface the cash-flow & debt metrics as “ratios” for convenience
                def _n(v):
                    try:
                ...
                # Depreciation & Amortization (splits or DepAmort fallback)
                dep_total = _dep_total_from_row(rowx)

                # Total Debt / Borrowings = Borrowings + OtherLoans + Leases (current & non-current)
                td_parts = [
                    _n(rowx.get("Borrowings")),
                    _n(rowx.get("OtherLoans")),
                    _n(rowx.get("LeaseLiabCurrent")),
                    _n(rowx.get("LeaseLiabNonCurrent")),
                ]
                td_total = float(np.nansum([p for p in td_parts if np.isfinite(p)])) if any(np.isfinite(p) for p in td_parts) else np.nan

                # ✅ Robust EBITDA for ALL years
                ebitda_val = derive_ebitda_from_row(rowx)

                r.update({
                    "CapEx":                       _n(rowx.get("CapEx")),
                    "Operating Cash Flow":         _n(rowx.get("CFO")),
                    "Depreciation & Amortization": dep_total,
                    "EBITDA":                      (float(ebitda_val) if np.isfinite(ebitda_val) else np.nan),
                    "Total Debt / Borrowings":     td_total,
                })
            ...
            try:
                ...
                if not q4.empty:
                    ...
                    # ---- ensure EBITDA (TTM) is filled even if calc_ratios() didn't
                    def _n(x):
                        try:
                    ...
                    # Depreciation & Amortization (TTM): prefer DepAmort in synth; else sum splits
                    da_ttm = _n(synth.get("DepAmort"))
                    if not np.isfinite(da_ttm):
                        da_ttm = float(np.nansum([
                            _n(synth.get("DepPPE")), _n(synth.get("DepInvProp")), _n(synth.get("DepROU"))
                        ]))

                    # EBIT (TTM): prefer OperatingProfit; else GrossProfit - Admin - Selling
                    op_ttm = _n(synth.get("OperatingProfit"))
                    if not np.isfinite(op_ttm):
                        gp  = _n(synth.get("GrossProfit"))
                        adm = _n(synth.get("AdminExpenses"))
                        sde = _n(synth.get("SellDistExpenses"))
                        if all(np.isfinite(v) for v in (gp, adm, sde)):
                            op_ttm = gp - adm - sde

                    # Start with whatever calc_ratios gave us
                    ebitda_ttm_val = _n(r_ttm.get("EBITDA"))

                    # Fill if missing: first EBIT + D&A, then NP + Fin + Tax + D&A
                    if not np.isfinite(ebitda_ttm_val):
                        if np.isfinite(op_ttm) and np.isfinite(da_ttm):
                            ebitda_ttm_val = op_ttm + da_ttm
                        else:
                            ebitda_ttm_val = float(np.nansum([
                                _n(synth.get("NetProfit")),
                                _n(synth.get("FinanceCosts")),
                                _n(synth.get("IncomeTax")),
                                da_ttm
                            ]))

                    # Store back so the ratios table picks them up
                    r_ttm["Depreciation & Amortization"] = da_ttm if np.isfinite(da_ttm) else np.nan
                    r_ttm["EBITDA"] = ebitda_ttm_val if np.isfinite(ebitda_ttm_val) else np.nan


                    # --- same convenience fields for TTM row (mirror Annual ratios table)
                    # (EBITDA is taken from r_ttm so it can be derived from EBIT + DepAmort)
                    # include a quarterly debt snapshot (Borrowings + OtherLoans + Leases)
                    td_vals = [
                        _last_q("Q_Borrowings"),
                        _last_q("Q_OtherLoans"),
                        _last_q("Q_LeaseLiabCurrent"),
                        _last_q("Q_LeaseLiabNonCurrent"),
                    ]
                    td_total_ttm = float(np.nansum([v for v in td_vals if pd.notna(v)])) if any(pd.notna(v) for v in td_vals) else np.nan

                    r_ttm.update({
                        "CapEx":                   synth.get("CapEx"),
                        "Operating Cash Flow":     synth.get("CFO"),
                        # (leave the two lines below OUT — we already set them above)
                        # "Depreciation & Amortization": ...,
                        # "EBITDA": ...,
                        "Total Debt / Borrowings": td_total_ttm,
                    })

                    r_ttm["Year"] = f"TTM {last_year}"

                    # keep for the JSON download later
                    ttm_detail_export = {
                        "label": r_ttm["Year"],
            ...
            # ---- Canonical ratios for charts: Years in rows, Metrics in columns
            ratio_df_canon = ratio_df.copy()

            # keep only numeric-year rows (drop TTM etc.), index as ints, sorted
            _idx = pd.to_numeric(ratio_df_canon.index, errors="coerce")
            ratio_df_canon = ratio_df_canon[_idx.notna()].copy()
            ratio_df_canon.index = _idx[_idx.notna()].astype(int)
            ratio_df_canon.sort_index(inplace=True)

            # defaults for chart state (do NOT overwrite on every rerun)
            _default_x = [str(int(y)) for y in ratio_df_canon.index]
            _default_metrics = ratio_df_canon.columns.tolist()
            st.session_state.setdefault(f"ann_ratio_x_{stock_name}", _default_x)
            st.session_state.setdefault(f"ann_ratio_metrics_{stock_name}", _default_metrics)
            ...
            if ratio_df.empty:
                ...
                if ratio_layout.startswith("Years"):
                    ...
                    # Respect the user's Year column reordering for the X-axis
                    _year_cols = pd.to_numeric(pd.Index(new_cols_r), errors="coerce")
                    years_for_x = [str(int(y)) for y in _year_cols.dropna().astype(int).tolist()]
                    if not years_for_x:  # fallback
                        years_for_x = [str(int(y)) for y in ratio_df_canon.index.tolist()]

                    st.session_state[f"ann_ratio_x_{stock_name}"] = years_for_x
                    # Metric options always come from columns of the canonical frame
                    st.session_state[f"ann_ratio_metrics_{stock_name}"] = ratio_df_canon.columns.tolist()
                    ...
                    # X-axis is always the real years from the canonical frame
                    st.session_state[f"ann_ratio_x_{stock_name}"] = [str(int(y)) for y in ratio_df_canon.index.tolist()]
                    # Respect the user's metric-column order for the dropdowns
            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                ...
                def fmt0(v):
                    return "–" if not (isinstance(v, (int, float)) and np.isfinite(v)) else f"{v:,.0f}"

                def fmt2(v):
                    return "–" if not (isinstance(v, (int, float)) and np.isfinite(v)) else f"{v:,.2f}"

                def fmt4(v):
                    return "–" if not (isinstance(v, (int, float)) and np.isfinite(v)) else f"{v:,.4f}"

                # ---- build choice list: numeric Years + explicit TTM label (if present)
                years_only = []
                for y in getattr(ratio_df, "index", []):
                    try:
                        years_only.append(int(str(y)))
                    except Exception:
                        # ignore non-numeric (e.g., "TTM 2025") in the index; we'll add TTM explicitly
                        pass
                year_labels = [str(y) for y in sorted(set(years_only))]

                ttm_label = (ttm_detail_export or {}).get("label")  # e.g. "TTM 2025"
                if ttm_label:
                    year_labels.append(ttm_label)
                ...
                if not year_labels:
                    ...
                    sel_label = st.selectbox(
                        "Year / TTM",
                        options=year_labels,
                        index=len(year_labels) - 1,  # default to the last entry (usually TTM or latest year)
                        key=f"ann_ratio_detail_year_{stock_name}",
                    )

                    # map "YYYY" -> annual row dict (match by YEAR VALUE, not by position)
                    annual_rec_by_year: dict[str, dict] = {}
                    if isinstance(annual, pd.DataFrame) and not annual.empty:
                        tmp = annual.copy()
                        if "Year" in tmp.columns:
                            tmp["__y__"] = pd.to_numeric(tmp["Year"], errors="coerce").astype("Int64")
                        else:
                            tmp["__y__"] = pd.to_numeric(tmp.index, errors="coerce").astype("Int64")
                        for _, row in tmp.iterrows():
                            if pd.notna(row["__y__"]):
                                annual_rec_by_year[str(int(row["__y__"]))] = row.to_dict()

                    # ---------- branch A: TTM ----------
                    if ttm_label and sel_label == ttm_label:
                        comp = (ttm_detail_export or {}).get("components", {}) or {}
                        ratios_from_ttm = (ttm_detail_export or {}).get("ratios", {}) or {}
                        ...
                        st.markdown(
                            f"- **EPS** = Net Profit ÷ Shares = {fmt0(npf)} ÷ {fmt0(sh)} = **{fmt4(eps)}**  \n"
                            f"- **ROE** = Net Profit ÷ Equity = {fmt0(npf)} ÷ {fmt0(eq)} = **{fmt2(roe)}%**  \n"
                            f"- **P/E** = Price ÷ EPS = {fmt4(price)} ÷ {fmt4(eps)} = **{fmt2(pe)}**  \n"
                            f"- **P/B** = (Price×Shares) ÷ Equity = {fmt0(mc)} ÷ {fmt0(eq)} = **{fmt2(pb)}**  \n"
                            f"- **Gross Margin** = Gross Profit ÷ Revenue = {fmt0(gp)} ÷ {fmt0(rev)} = **{fmt2(gpm)}%**  \n"
                            f"- **Net Margin** = Net Profit ÷ Revenue = {fmt0(npf)} ÷ {fmt0(rev)} = **{fmt2(npm)}%**  \n"
                            f"- **Current Ratio** = Current Assets ÷ Current Liabilities = {fmt0(ca)} ÷ {fmt0(cl)} = **{fmt2(curr)}×**  \n"
                            f"- **Quick Ratio** ≈ (CA − Inventories) ÷ CL = {fmt0(ca - inv if np.isfinite(ca) and np.isfinite(inv) else np.nan)} ÷ {fmt0(cl)} = **{fmt2(quick)}×**  \n"
                            f"- **Dividend Yield** ≈ Dividend/share ÷ Price = {fmt4(dps)} ÷ {fmt4(price)} = **{fmt2(dy)}%**  \n"
                            f"- **BVPS** = Equity ÷ Shares = {fmt0(eq)} ÷ {fmt0(sh)} = **{fmt4(bvps)}**  \n"
                            + (f"- **NTA / share** = (Equity − Intangibles) ÷ Shares = {fmt0(eq - inta)} ÷ {fmt0(sh)} = **{fmt4(nta)}**"
                            if np.isfinite(inta) else "")
                        )

                        # ---- extras (TTM versions) — with "what + what"
                        cfo_ttm   = n(comp.get("CFO"))
                        capex_ttm = n(comp.get("CapEx"))

                        # D&A (sum splits; fallback DepAmort)
                        da_parts = []
                        for label, key in [("Dep PPE", "DepPPE"), ("Dep InvProp", "DepInvProp"), ("Dep ROU", "DepROU")]:
                            v = n(comp.get(key))
                            if np.isfinite(v):
                                da_parts.append((label, v))
                        if not da_parts:
                            v = n(comp.get("DepAmort"))
                            if np.isfinite(v):
                                da_parts.append(("DepAmort", v))
                        da_ttm = float(np.nansum([v for _, v in da_parts])) if da_parts else np.nan
                        da_str = " + ".join([f"{k}={fmt0(v)}" for k, v in da_parts]) if da_parts else "–"

                        # EBITDA (1) reported; (2) EBIT + D&A; (3) NP + Fin + Tax + D&A
                        raw_ebitda = n(ratios_from_ttm.get("EBITDA"))
                        op = n(comp.get("OperatingProfit"))
                        ebit_src = "OperatingProfit"
                        if not np.isfinite(op):
                            gp_  = n(comp.get("GrossProfit"))
                            adm_ = n(comp.get("AdminExpenses"))
                            sde_ = n(comp.get("SellDistExpenses"))
                            if all(np.isfinite(x) for x in (gp_, adm_, sde_)):
                                op = gp_ - adm_ - sde_
                                ebit_src = "GrossProfit − Admin − Selling"

                        if np.isfinite(raw_ebitda) and raw_ebitda != 0:
                            ebitda_line = f"EBITDA (reported) = **{fmt0(raw_ebitda)}**"
                        elif np.isfinite(op) and np.isfinite(da_ttm):
                            ebitda_line = f"EBITDA = EBIT + D&A = ({ebit_src}: {fmt0(op)}) + ({fmt0(da_ttm)}) = **{fmt0(op + da_ttm)}**"
                        else:
                            npf_ = n(comp.get("NetProfit"))
                            fin_ = n(comp.get("FinanceCosts"))
                            tax_ = n(comp.get("IncomeTax"))
                            if not np.isfinite(tax_):
                                for alt in ("Tax", "TaxExpense", "Income Tax Expense"):
                                    t = n(comp.get(alt))
                                    if np.isfinite(t):
                                        tax_ = t
                                        break
                            parts = [("NP", npf_), ("Fin", fin_), ("Tax", tax_), ("D&A", da_ttm)]
                            shown = [f"{k}={fmt0(v)}" for k, v in parts if np.isfinite(v)]
                            total = float(np.nansum([v for _, v in parts if np.isfinite(v)])) if shown else np.nan
                            ebitda_line = f"EBITDA = " + " + ".join(shown) + f" = **{fmt0(total)}**" if shown else "EBITDA = –"

                        # Debt latest (quarterly snapshot)
                        def _last_q(col):
                            s = pd.to_numeric(quarterly.get(col), errors="coerce").dropna()
                            return float(s.iloc[-1]) if not s.empty else np.nan

                        bq  = _last_q("Q_Borrowings")
                        oq  = _last_q("Q_OtherLoans")
                        l1q = _last_q("Q_LeaseLiabCurrent")
                        l2q = _last_q("Q_LeaseLiabNonCurrent")
                        debt_parts = [("Borrowings", bq), ("OtherLoans", oq), ("Lease(Current)", l1q), ("Lease(Non-current)", l2q)]
                        debt_shown = [f"{k}={fmt0(v)}" for k, v in debt_parts if np.isfinite(v)]
                        debt_sum   = float(np.nansum([v for _, v in debt_parts if np.isfinite(v)])) if debt_shown else np.nan

                        st.markdown(
                            f"- **CapEx (TTM)** = **{fmt0(capex_ttm)}**  \n"
                            f"- **Operating Cash Flow (TTM)** = **{fmt0(cfo_ttm)}**  \n"
                            f"- **Depreciation & Amortization (TTM)** = {da_str} = **{fmt0(da_ttm)}**  \n"
                            f"- {ebitda_line}  \n"
                            f"- **Total Debt / Borrowings (latest)** = "
                            + ((" + ".join(debt_shown) + f" = **{fmt0(debt_sum)}**") if debt_shown else "–")
                        )
                    ...
                    # ---------- branch B: specific YEAR ----------
                    else:
                        raw = annual_rec_by_year.get(sel_label, {}) if sel_label else {}
                        ...
                        # ---- extras (Year versions) — with "what + what"
                        cfo_y   = n(raw.get("CFO"))
                        capex_y = n(raw.get("CapEx"))
                        ...
                        for label, key in [("Dep PPE", "DepPPE"), ("Dep InvProp", "DepInvProp"), ("Dep ROU", "DepROU")]:
                            v = n(raw.get(key))
                            if np.isfinite(v):
                                da_parts_y.append((label, v))
                        if not da_parts_y:
                            v = n(raw.get("DepAmort"))
                            if np.isfinite(v):
                                da_parts_y.append(("DepAmort", v))
                        da_y = float(np.nansum([v for _, v in da_parts_y])) if da_parts_y else np.nan
                        da_str_y = " + ".join([f"{k}={fmt0(v)}" for k, v in da_parts_y]) if da_parts_y else "–"

                        # EBITDA (1) reported; (2) EBIT + D&A; (3) NP + Fin + Tax + D&A
                        raw_ebitda_y = n(raw.get("EBITDA"))
                        opy = n(raw.get("OperatingProfit"))
                        ebit_src_y = "OperatingProfit"
                        if not np.isfinite(opy):
                            gp_  = n(raw.get("GrossProfit"))
                            adm_ = n(raw.get("AdminExpenses"))
                            sde_ = n(raw.get("SellDistExpenses"))
                            if all(np.isfinite(x) for x in (gp_, adm_, sde_)):
                                opy = gp_ - adm_ - sde_
                                ebit_src_y = "GrossProfit − Admin − Selling"

                        if np.isfinite(raw_ebitda_y) and raw_ebitda_y != 0:
                            ebitda_line_y = f"EBITDA (reported) = **{fmt0(raw_ebitda_y)}**"
                        elif np.isfinite(opy) and np.isfinite(da_y):
                            ebitda_line_y = f"EBITDA = EBIT + D&A = ({ebit_src_y}: {fmt0(opy)}) + ({fmt0(da_y)}) = **{fmt0(opy + da_y)}**"
                        else:
                            npf_ = n(raw.get("NetProfit"))
                            fin_ = n(raw.get("FinanceCosts"))
                            tax_ = n(raw.get("IncomeTax"))
                            if not np.isfinite(tax_):
                                for alt in ("Tax", "TaxExpense", "Income Tax Expense"):
                                    t = n(raw.get(alt))
                                    if np.isfinite(t):
                                        tax_ = t
                                        break
                            parts = [("NP", npf_), ("Fin", fin_), ("Tax", tax_), ("D&A", da_y)]
                            shown = [f"{k}={fmt0(v)}" for k, v in parts if np.isfinite(v)]
                            total = float(np.nansum([v for _, v in parts if np.isfinite(v)])) if shown else np.nan
                            ebitda_line_y = f"EBITDA = " + " + ".join(shown) + f" = **{fmt0(total)}**" if shown else "EBITDA = –"

                        # Total Debt / Borrowings = Borrowings + OtherLoans + Leases
                        b  = n(raw.get("Borrowings"))
                        o  = n(raw.get("OtherLoans"))
                        l1 = n(raw.get("LeaseLiabCurrent"))
                        l2 = n(raw.get("LeaseLiabNonCurrent"))
                        debt_parts_y = [("Borrowings", b), ("OtherLoans", o), ("Lease(Current)", l1), ("Lease(Non-current)", l2)]
                        debt_shown_y = [f"{k}={fmt0(v)}" for k, v in debt_parts_y if np.isfinite(v)]
                        debt_sum_y   = float(np.nansum([v for _, v in debt_parts_y if np.isfinite(v)])) if debt_shown_y else np.nan

                        st.markdown(
                            f"- **CapEx** = **{fmt0(capex_y)}**  \n"
                            f"- **Operating Cash Flow** = **{fmt0(cfo_y)}**  \n"
                            f"- **Depreciation & Amortization** = {da_str_y} = **{fmt0(da_y)}**  \n"
                            f"- {ebitda_line_y}  \n"
                            f"- **Total Debt / Borrowings** = "
                            + ((" + ".join(debt_shown_y) + f" = **{fmt0(debt_sum_y)}**") if debt_shown_y else "–")
                        )
            ...
            **Chosen horizon for forecast**: {win} year(s) (from latest year {last_year})
            ...
            # Defaults ALWAYS from the canonical frame
            ratio_x = st.session_state.get(
                f"ann_ratio_x_{stock_name}",
                [str(int(y)) for y in ratio_df_canon.index.tolist()]
            )
            ratio_metrics = st.session_state.get(
                f"ann_ratio_metrics_{stock_name}",
                ratio_df_canon.columns.tolist()
            )

            def ratio_series_getter(metric_name):
                if metric_name not in ratio_df_canon.columns:
                    return None
                s = pd.to_numeric(ratio_df_canon[metric_name], errors="coerce")
                # align to chosen X order (string years)
                s.index = [str(int(y)) for y in ratio_df_canon.index]
                s = s.reindex(ratio_x)
                return None if s.dropna().empty else s.values
...
with tabs[1]:
    ...
    for _, row in quarterly.iterrows():
        ...
        # map quarterly fields into the annual-style keys the helper expects
        row_for_ebitda = {
            "GrossProfit":        rowx.get("Q_GrossProfit") or rowx.get("GrossProfit"),
            "AdminExpenses":      rowx.get("Q_AdminExpenses") or rowx.get("AdminExpenses"),
            "SellDistExpenses":   rowx.get("Q_SellDistExpenses") or rowx.get("SellDistExpenses"),
            "OperatingProfit":    rowx.get("Q_OperatingProfit") or rowx.get("Q_EBIT") or rowx.get("OperatingProfit"),
            "NetProfit":          rowx.get("Q_NetProfit") or rowx.get("NetProfit"),
            "FinanceCosts":       rowx.get("Q_FinanceCosts") or rowx.get("FinanceCosts"),
            "IncomeTax":          rowx.get("Q_IncomeTax") or rowx.get("Q_Tax") or rowx.get("IncomeTax"),
            "DepPPE":             rowx.get("Q_DepPPE"),
            "DepInvProp":         rowx.get("Q_DepInvProp"),
            "DepROU":             rowx.get("Q_DepROU"),
            "DepAmort":           rowx.get("Q_DepAmort"),
            "EBITDA":             rowx.get("Q_EBITDA") or rowx.get("EBITDA"),
        }

        dep_total = _dep_total_from_row(row_for_ebitda)
        ebitda_val = derive_ebitda_from_row(row_for_ebitda)

        # debt: keep your current definition; include leases if you want
        total_debt = 0.0
        for k in ("Q_Borrowings", "Q_OtherLoans"):
            v = _n(rowx.get(k))
            if np.isfinite(v):
                total_debt += v
        total_debt = (np.nan if total_debt == 0.0 else total_debt)
