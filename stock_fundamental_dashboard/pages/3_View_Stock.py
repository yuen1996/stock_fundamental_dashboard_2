        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                ...
                raw_ann_json = {
                    ...
                    "type": "annual_raw_row",
                    "year": int(sel_year),
                    "row": (annual[annual["Year"] == sel_year].tail(1).replace({pd.NA: None, np.nan: None}).to_dict("records")[0]
                            if sel_year in annual["Year"].values else {}),
                }
                _download_json_button("📥 Download This Year's Raw Row (JSON)",
                                    raw_ann_json, f"{stock_name}_{sel_year}_annual_raw.json",
                                    key=f"ann_row_{stock_name}_{sel_year}")
