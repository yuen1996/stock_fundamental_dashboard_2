    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            for _, row in annual.iterrows():
                ...
                # --- Improve: build D&A and robust EBITDA BEFORE calc_ratios so downstream uses it
                def _n(v):
                    try:
                ...
                dep_total = _n(rowx.get("DepAmort"))
                if not np.isfinite(dep_total):
                    parts = [_n(rowx.get("DepPPE")), _n(rowx.get("DepInvProp")), _n(rowx.get("DepROU"))]
                    dep_total = float(np.nansum([p for p in parts if np.isfinite(p)])) if any(np.isfinite(p) for p in parts) else np.nan
                ...
                if not np.isfinite(ebitda):
                    ebit = _n(rowx.get("EBIT"))
                    if not np.isfinite(ebit):
                        ebit = _n(rowx.get("OperatingProfit"))
