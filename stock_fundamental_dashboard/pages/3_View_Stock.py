        ...
        with tabs[0]:
            ...
            **Chosen horizon for forecast**: {win} year(s) (from latest year {last_year})
            ...
            st.markdown(
                ...
                '<div class="d">Read-only evaluation (same rules as the Systematic Decision page)</div></div>',
                unsafe_allow_html=True
            )
            ...
            if annual_rows_for_eval.empty:
                ...
                # Use the same ruleset selection as the Systematic Decision page
                all_rules = list(rules.RULESETS)
                default_idx = all_rules.index("VQGM") if "VQGM" in all_rules else 0
                strategy = st.selectbox(
                    "Ruleset",
                    all_rules,
                    index=default_idx,
                    key=f"ruleset_{stock_name}",
                )

                # Match the page’s metric logic:
                # • VQGM → TTM metrics (needs all rows for this stock + current price)
                # • Others → latest annual point-in-time ratios
                if strategy == "VQGM":
                    metrics = calculations.compute_ttm(stock, current_price=cur_val) or {}
                else:
                ...
                with cB:
                    ...
                    # Keep the checks simple, but show them in an expander like your screenshot
