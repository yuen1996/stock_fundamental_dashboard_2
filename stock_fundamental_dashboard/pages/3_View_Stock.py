    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            for _, row in annual.iterrows():
                ...
                # Make valuation metrics use today's price (P/E, P/B, DY, EV/EBITDA, etc.)
                try:
                ...
                def _pick(row, *, exact_names=(), fuzzy_all=()):
                    ...
                    if fuzzy_all:
                        for k, v0 in row.items():
                            k2 = str(k).lower()
                            if k2.startswith("q_"):  # skip quarterly columns in annual loop
                                continue
                ...
                # Prefer a single combined field (various aliases); else sum the splits.
                dep_total = _pick(
                    rowx,
                    exact_names=[
                ...
                # ---------- Other pieces used by EBITDA fallback ----------
                fin = _pick(
                    rowx,
                    exact_names=[
                ...
                if np.isfinite(fin):
                    fin = abs(fin)   # add-back should be positive
                ...
                # ---------- EBITDA build ----------
                # 1) use raw EBITDA if given
                # 2) else EBIT/OperatingProfit + DepAmort
                # 3) else NP + Tax + FinanceCosts + DepAmort
                ...
                # Run your ratio engine on the enriched row
