    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("Download — Annual Raw (JSON)", expanded=False):
                # disp_num is the table you just rendered above (both layouts end up in disp_num)
                ann_payload = {
                    "stock":  stock_name,
                    "type":   "annual_raw",
                    "layout": "Years→columns" if ann_raw_layout.startswith("Years") else "Fields→columns",
                    "visible_table": (disp_num.reset_index().to_dict(orient="records") if isinstance(disp_num, pd.DataFrame) else []),
                    "raw_rows": annual.replace({pd.NA: None}).to_dict(orient="records"),
                }
                _download_json_button("📥 Download Annual Raw (JSON)", ann_payload, f"{stock_name}_annual_raw.json")
            ...
            with st.expander("Download — TTM Snapshot (JSON)", expanded=False):
                ttm_payload = {"stock": stock_name, "type": "ttm_snapshot", **(ttm or {})}
                _download_json_button(
                    "📥 Download TTM Snapshot (JSON)",
                    ttm_payload,
            ...
            if show_cf:
                ...
                cfw_payload = {
                    "stock": stock_name,
                    "type": "cash_flow_wealth",
                    "ttm": (ttm or {}),
                }
                _download_json_button("📥 Download Cash-Flow Wealth (JSON)", cfw_payload, f"{stock_name}_cashflow_wealth.json")
            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                ...
                # Optional: raw annual row (the one you’re inspecting here)
                raw_ann_json = {
                    "stock": stock_name,
                    "type": "annual_raw_row",
                    "year": int(sel_year),
                    "row": raw_rec[0] if raw_rec else {},
                }
