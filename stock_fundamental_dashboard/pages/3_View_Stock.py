    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("Download — Annual Raw (JSON)", expanded=False):
                # safer conversion (turn NaN/NA into None everywhere)
                def _records(df):
                    try:
                        return (
                ...
                ann_payload = {
                    ...
                    "visible_table": _records(disp_num.reset_index()) if isinstance(disp_num, pd.DataFrame) else [],
            ...
            with st.expander("Show calculation details — TTM", expanded=False):
                ...
                # ——— Download TTM snapshot (inside this expander) ———
                ttm_payload = {"stock": stock_name, "type": "ttm_snapshot", **(ttm or {})}
                _download_json_button("📥 Download TTM Snapshot (JSON)", ttm_payload, f"{stock_name}_ttm.json")
