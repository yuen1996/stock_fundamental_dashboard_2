    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            for _, row in annual.iterrows():
                ...
                # --- Robust D&A + EBITDA build (aliases + sign fixes) -------------------------
                def _n(v):
                    try:
                ...
                def _pick(row, *, exact_names=(), fuzzy_all=()):
                    """
                    Pick first finite value by exact alias list,
                    else by fuzzy 'all substrings present' in the key (case-insensitive).
                    """
                    # exact aliases (prefer non-zero first, then allow zero)
                    for require_nonzero in (True, False):
                        for name in exact_names:
                            if name in row:
                                val = _n(row.get(name))
                                if np.isfinite(val) and (abs(val) > 0 or not require_nonzero):
                                    return val
                    # fuzzy aliases
                    if fuzzy_all:
                        for k, v0 in row.items():
                            k2 = str(k).lower()
                            if k2.startswith("q_"):  # skip quarterly fields in annual loop
                                continue
                            if all(s in k2 for s in fuzzy_all):
                                val = _n(v0)
                                if np.isfinite(val):
                                    return val
                    return np.nan

                # 1) Depreciation & Amortization (prefer single field; else sum splits; fuzzy too)
                dep_total = _pick(
                    rowx,
                    exact_names=[
                        "DepAmort", "Depreciation & Amortization", "Depreciation & Amortisation",
                        "DepreciationAndAmortization", "Depreciation and amortisation"
                    ],
                    fuzzy_all=("depreci", "amort"),
                )
                if not np.isfinite(dep_total):
                    dep_parts = [
                        _pick(rowx,
                            exact_names=["DepPPE", "Depreciation of property, plant and equipment"],
                            fuzzy_all=("depreci", "property")),
                        _pick(rowx,
                            exact_names=["DepInvProp", "Depreciation of investment property"],
                            fuzzy_all=("depreci", "investment")),
                        _pick(rowx,
                            exact_names=["DepROU", "Depreciation of right-of-use assets", "ROU depreciation"],
                            fuzzy_all=("depreci", "right-of-use")),
                    ]
                    if any(np.isfinite(p) for p in dep_parts):
                        dep_total = float(np.nansum([p for p in dep_parts if np.isfinite(p)]))

                # D&A should be a positive add-back
                if np.isfinite(dep_total):
                    dep_total = abs(dep_total)
                    rowx["DepAmort"] = dep_total

                # 2) Finance cost / tax / EBIT / NP (with broad aliases)
                fin = _pick(
                    rowx,
                    exact_names=[
                        "FinanceCosts", "Finance costs", "Finance cost", "Finance expenses",
                        "InterestExpense", "Interest Expense", "Finance charges"
                    ],
                    fuzzy_all=("finance", "cost"),
                )
                if np.isfinite(fin):
                    fin = abs(fin)  # add-back is positive

                tax = _pick(
                    rowx,
                    exact_names=["IncomeTax", "Income Tax", "Income Tax Expense", "Tax expense", "Taxation"],
                    fuzzy_all=("tax",),
                )

                ebit = _pick(
                    rowx,
                    exact_names=["EBIT", "OperatingProfit", "Operating Profit", "Profit before interest and tax"],
                    fuzzy_all=("operating", "profit"),
                )

                npf = _pick(
                    rowx,
                    exact_names=["NetProfit", "Net Income", "Profit for the year"],
                    fuzzy_all=("net", "profit"),
                )

                # 3) EBITDA build:
                #    (i) prefer raw EBITDA if present,
                #    (ii) else EBIT/Operating Profit + DepAmort,
                #    (iii) else Net Profit + Tax + Finance Costs + DepAmort.
                ebitda = _n(rowx.get("EBITDA"))
                ...
                # now run your ratio engine
                r = calculations.calc_ratios(rowx)

                # --- ADD: surface the cash-flow & debt metrics as “ratios” for convenience
                # Total Debt / Borrowings = Borrowings + OtherLoans + LeaseLiabCurrent + LeaseLiabNonCurrent
                ...
                r.update({
                    ...
                    "EBITDA":                      (ebitda if np.isfinite(ebitda) else _n(rowx.get("EBITDA"))),
