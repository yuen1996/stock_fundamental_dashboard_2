    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("📥 Download — Annual Raw (JSON)", expanded=False):
                ...
                # --- Enriched JSON payload for export (includes numeric matrix + TTM column values)
                # vis            -> the table you're showing to the user
                # ann_numeric    -> the numeric matrix (Fields x Years) we built above
                # new_cols       -> the current column order the user sees (includes "TTM <year>" if present)
                matrix_df = ann_numeric.copy()
                # Keep the same visible order (so JSON matches what user sees in the table area)
                try:
                    matrix_df = matrix_df[new_cols]
                except Exception:
                    pass

                # Find the TTM column if one exists (e.g. "TTM 2025")
                ttm_colname = next((c for c in matrix_df.columns if isinstance(c, str) and str(c).startswith("TTM ")), None)
                ttm_values = (
                    { nice_label(k): (None if pd.isna(v) else float(v))
                    for k, v in matrix_df[ttm_colname].items() }
                    if ttm_colname and ttm_colname in matrix_df.columns
                    else {}
                )

                # ---- Optional TTM breakdown (last 4 quarters) to show above the button
                raw_ttm_detail = None
                try:
                    q = quarterly.copy()
                    q["Year"] = pd.to_numeric(q["Year"], errors="coerce")
                    ...
                    if not q4.empty:
                        ...
                        def _sum(col):
                            return float(pd.to_numeric(q4.get(col), errors="coerce").dropna().sum()) if col in q4.columns else None
                        def _last(col):
                            s = pd.to_numeric(q4.get(col), errors="coerce").dropna() if col in q4.columns else pd.Series(dtype=float)
                            return float(s.iloc[-1]) if not s.empty else None

                        raw_ttm_detail = {
                            "label": f"TTM (to {last_year})",
                            "quarters_used": [(int(y), str(qt)) for y, qt in zip(q4["Year"], q4["Quarter"])],
                            "components": {
                                "TTM Revenue": _sum("Q_Revenue"),
                                "TTM Net Profit": _sum("Q_NetProfit"),
                                "TTM Gross Profit": _sum("Q_GrossProfit"),
                                "TTM Cost Of Sales": _sum("Q_CostOfSales"),
                                "CFO (TTM)": _sum("Q_CFO"),
                                "CapEx (TTM)": _sum("Q_CapEx"),
                                "Shares (latest)": _last("Q_NumShares"),
                                "Current Asset (latest)": _last("Q_CurrentAsset"),
                                "Current Liability (latest)": _last("Q_CurrentLiability"),
                                "Price (latest)": _last("Q_EndQuarterPrice"),
                            },
                        }
                except Exception:
                    raw_ttm_detail = None

                # show a sub-expander with the components
                if raw_ttm_detail:
                    with st.expander(f"Show calculation of {raw_ttm_detail['label']}", expanded=False):
                        st.json({
                            k: (None if (v is None or (isinstance(v,float) and (np.isnan(v) or np.isinf(v)))) else float(v))
                            for k, v in (raw_ttm_detail.get("components") or {}).items()
                        })
                ...
                ann_payload = {
                    ...
                    "ttm_detail": raw_ttm_detail,


                    # what the user is looking at (already styled/reordered)
                    "visible_table": _records(vis),

                    # the full numeric matrix the calculations used (fields-as-rows, years-as-columns)
                    "matrix_fields_columns": _records(matrix_df),

                    # extracted TTM column (if present) as a convenient dict { Field -> value }
                    "ttm": {
                        "column": ttm_colname,
                        "values": {k: (None if pd.isna(v) else float(v)) for k, v in ttm_values.items()} if ttm_values else {},
                    },
                }
            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                # Build a numeric-only year list (ignore 'TTM ...' etc. for this panel)
                years_only = []
                for y in ratio_df.index:
                    try:
                        ...
                        pass
                year_options = sorted(set(years_only))

                if not year_options:
                    st.info("No annual history (TTM is not shown in this details panel).")
                else:
                    sel_year = st.selectbox(
                        "Year",
                        options=year_options,
                        index=max(0, len(year_options) - 1),
                        key=f"ann_ratio_detail_year_{stock_name}",
                    )

                    # Pull the raw annual row for that year (robust match)
                    yr_col = pd.to_numeric(annual.get("Year", pd.Series(index=annual.index)), errors="coerce")
                    raw_rec = annual[yr_col == int(sel_year)].tail(1).to_dict("records")
                    ...
                    st.markdown("---")
                    # Single canonical ratios export (with both orientations + TTM detail)
                    try:
                        _yr = ratio_df.copy()
                        _mx = _yr.T
                    except Exception:
                    ...
                    ratio_payload = {
                        ...
                        "industry": industry,
                        "industry_bucket": industry_bucket,
                        "layout": ratio_layout,                # whatever radio was selected
                        "year_rows":   _records(_yr.reset_index()),   # Year as first column
                        "metric_rows": _records(_mx.reset_index()),   # Metric as first column
                        "ttm_detail": (ttm_detail_export or {}),      # from the TTM row builder above
                    }
                    _download_json_button(
