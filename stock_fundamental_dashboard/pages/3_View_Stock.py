...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            # --- pull the latest FY row from `annual`
            try:
            ...
            # --- FY inputs (prefer annual raw fields)
            ...
            # Price for FY block: prefer annual SharePrice; fallback to live price_now
            ...
            try:
                da_fy = _dep_total_from_row(_raw)  # your helper if available
            except Exception:
            ...
            if not np.isfinite(ebitda_fy) and np.isfinite(op) and np.isfinite(da_fy):
                ebitda_fy = op + da_fy

            fincost = first_from_raw("FinanceCosts", "Finance Costs", "Finance cost", "Finance Expense", "Finance expenses")
            if not np.isfinite(ebitda_fy):
                tax_ = first_from_raw("IncomeTax", "Tax", "TaxExpense", "Income Tax Expense")
                if any(np.isfinite(x) for x in (npf, fincost, tax_, da_fy)):
                    ebitda_fy = float(np.nansum([npf, fincost, tax_, da_fy]))

            # Market cap (est.): prefer existing 'market_cap'; else price_used×shares; else P/S×FY revenue
            market_cap_est = n(locals().get("market_cap", np.nan))
            if not np.isfinite(market_cap_est):
                if np.isfinite(price_used) and np.isfinite(shares):
                    market_cap_est = price_used * shares
                else:
                    try:
                        ps_ttm = n((ttm or {}).get("P/S (TTM)")) if 'ttm' in locals() and ttm else np.nan
                    except Exception:
                        ps_ttm = np.nan
                    if np.isfinite(ps_ttm) and np.isfinite(rev)):
                        market_cap_est = ps_ttm * rev

            # Debt & cash: prefer latest quarter snapshot if available; else construct from annual
            debt_latest_val = n(locals().get("debt_latest", np.nan))
            cash_latest_val = n(locals().get("cash_latest", np.nan))
            if not np.isfinite(debt_latest_val) or not np.isfinite(cash_latest_val):
                b  = first_from_raw("Borrowings", "Loans", "BankBorrowings")
                o  = first_from_raw("OtherLoans", "Other Borrowings")
                l1 = first_from_raw("LeaseLiabCurrent", "Lease(Current)")
                l2 = first_from_raw("LeaseLiabNonCurrent", "Lease(Non-current)")
                debt_fallback = float(np.nansum([b, o, l1, l2]))
                cash_fallback = first_from_raw("Cash", "Cash&Equivalents", "CashAndCashEquivalents")
                if not np.isfinite(debt_latest_val): debt_latest_val = debt_fallback
                if not np.isfinite(cash_latest_val): cash_latest_val = cash_fallback
            ...
            net_cash_debt = (cash_latest_val if np.isfinite(cash_latest_val) else 0.0) - (debt_latest_val if np.isfinite(debt_latest_val) else 0.0)
            net_cash_mc   = (net_cash_debt / market_cap_est * 100.0) if (np.isfinite(net_cash_debt) and np.isfinite(market_cap_est) and market_cap_est != 0) else np.nan
            icov          = (ebitda_fy / fincost) if (np.isfinite(ebitda_fy) and np.isfinite(fincost) and fincost != 0) else np.nan
            debt_to_fcf   = (debt_latest_val / fcf) if (np.isfinite(debt_latest_val) and np.isfinite(fcf) and fcf > 0) else np.nan

            # ---- UI tiles (FY latest) ----
            r1c1, r1c2, r1c3, r1c4 = st.columns(4)
            with r1c1:
                st.metric(f"FY{_latest_year} CFO", _fmt2(cfo, 0))
                st.metric("Cash Conversion (CFO/NP)", f"{_fmt2((cfo / npf * 100.0) if (np.isfinite(cfo) and np.isfinite(npf) and npf != 0) else np.nan, 2)}%")
            with r1c2:
                st.metric(f"FY{_latest_year} CapEx", _fmt2(capex, 0))
                st.metric("Debt / FCF (yrs)", _fmt2(debt_to_fcf, 2))
            with r1c3:
                st.metric(f"FY{_latest_year} FCF", _fmt2(fcf, 0))
                st.metric("FCF / Share (FY)", _fmt2(fcf_ps, 4))
            with r1c4:
                st.metric("FCF Margin (FY)", f"{_fmt2(fcf_margin, 2)}%")
                st.metric("FCF Yield (FY)", f"{_fmt2(fcf_yield, 2)}%")

            r2c1, r2c2, r2c3, r2c4 = st.columns(4)
            with r2c1:
                st.metric("Net Cash (Debt)", _fmt2(net_cash_debt, 0))
            with r2c2:
                st.metric("Net Cash / MC", f"{_fmt2(net_cash_mc, 2)}%")
            with r2c3:
            ...
            with st.expander(f"Show calculation details — Cash Flow Wealth (FY{_latest_year})", expanded=False):
                st.markdown(f"**FY {_latest_year}**")
                st.markdown(
                    ...
                    f"- **FCF / Share** = FCF ÷ Shares = {(fcf if np.isfinite(fcf) else np.nan):,.0f} ÷ {(shares if np.isfinite(shares) else np.nan):,.0f} = **{(fcf_ps if np.isfinite(fcf_ps) else np.nan):,.4f}**  \n"
                    f"- **FCF Margin** = FCF ÷ Revenue = {(fcf if np.isfinite(fcf) else np.nan):,.0f} ÷ {(rev if np.isfinite(rev) else np.nan):,.0f} = **{(fcf_margin if np.isfinite(fcf_margin) else np.nan):,.2f}%**  \n"
                    f"- **FCF Yield** = FCF ÷ Market Cap = {(fcf if np.isfinite(fcf) else np.nan):,.0f} ÷ {(market_cap_est if np.isfinite(market_cap_est) else np.nan):,.0f} = **{(fcf_yield if np.isfinite(fcf_yield) else np.nan):,.2f}%**  \n"
                    f"- **Net Cash (Debt)** = Cash − Debt = {(cash_latest_val if np.isfinite(cash_latest_val) else np.nan):,.0f} − {(debt_latest_val if np.isfinite(debt_latest_val) else np.nan):,.0f} = **{(net_cash_debt if np.isfinite(net_cash_debt) else np.nan):,.0f}**  \n"
                    f"- **Net Cash / MC** = Net Cash ÷ Market Cap = {(net_cash_debt if np.isfinite(net_cash_debt) else np.nan):,.0f} ÷ {(market_cap_est if np.isfinite(market_cap_est) else np.nan):,.0f} = **{(net_cash_mc if np.isfinite(net_cash_mc) else np.nan):,.2f}%**  \n"
                    + (
                        ...
                        if np.isfinite(fincost) and fincost != 0 else "- **Interest Coverage** = –"
                    )
                )
                ...
                fy_payload = {
                    "stock": stock_name,
                    "type": "cashflow_wealth_fy",
                    "year": _latest_year,
                    "inputs": {
                        "cfo": cfo, "capex": capex, "fcf": fcf, "revenue": rev,
                        "shares": shares, "price": price_used, "market_cap_est": market_cap_est,
                        "debt_latest": debt_latest_val, "cash_latest": cash_latest_val,
                        "ebitda_fy": ebitda_fy, "finance_costs": fincost
                    },
                    "derived": {
                        "fcf_ps": fcf_ps, "fcf_margin_%": fcf_margin, "fcf_yield_%": fcf_yield,
                        "net_cash_debt": net_cash_debt, "net_cash_mc_%": net_cash_mc,
                        "interest_coverage": icov, "debt_to_fcf_years": debt_to_fcf
                    }
                }
                _download_json_button("📥 Download Cash-Flow Wealth (FY) JSON", fy_payload, f"{stock_name}_cfw_fy.json", key=f"cfw_fy_{stock_name}")
