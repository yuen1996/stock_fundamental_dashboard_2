        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — Cash Flow Wealth", expanded=False):
                ...
                st.markdown(
                    ...
                    f"- **Debt / FCF (yrs)** = **{(debt_fcf_year if np.isfinite(debt_fcf_year) else np.nan):,.2f}**  \n"
                ...
                fy_payload = {
                    "stock": stock_name,
                    "type": "cashflow_wealth_detail_fy",
                    "selected": _label_yr,
                    "inputs": {
                        "cfo": cfo, "capex": capex, "fcf": fcf, "revenue": rev,
                        "shares": shares, "price": price, "market_cap_est": market_cap_est,
                        "net_cash": net_cash, "ebitda_fy": ebitda_fy, "finance_costs": fincost
                    },
                    "derived": {
                        "debt_over_fcf_years": debt_fcf_year,
                        "fcf_margin_pct": fcf_margin,
                        "fcf_per_share": fcf_ps,
                        "fcf_yield_pct": fcf_yield,
                        "netcash_over_mc_pct": netcash_mc,
                        "interest_coverage": icov,
                    },
                }
                _download_json_button(
                    "📥 Download Cash Flow Wealth (FY) — details (JSON)",
                    fy_payload,
                    f"{stock_name}_cashflow_wealth_fy_detail.json",
                    key=f"cfw_fy_detail_{stock_name}",
                )
