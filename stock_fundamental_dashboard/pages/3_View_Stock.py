...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            try:
                da_fy = _dep_total_from_row(_raw)  # uses your helper if available
            except Exception:
            ...
            # Market cap (est.): use live vars if present; otherwise price × shares; last resort P/S × FY revenue
            ...
            if not np.isfinite(market_cap_est):
                if np.isfinite(price_now_safe) and np.isfinite(shares):
                    market_cap_est = price_now_safe * shares
                else:
                    try:
            ...
            # Debt & cash: prefer latest quarter snapshot if available; else build from annual pieces
            debt_latest_val = n(locals().get("debt_latest", np.nan))
            cash_latest_val = n(locals().get("cash_latest", np.nan))
            if not np.isfinite(debt_latest_val):
                _b = first_from_raw("Borrowings")
                _o = first_from_raw("OtherLoans", "Other Loans")
                _l1 = first_from_raw("LeaseLiabCurrent")
                _l2 = first_from_raw("LeaseLiabNonCurrent")
                _pieces = [x for x in (_b, _o, _l1, _l2) if np.isfinite(x)]
                debt_latest_val = float(np.nansum(_pieces)) if _pieces else np.nan
            if not np.isfinite(cash_latest_val):
                cash_latest_val = first_from_raw("CashAndCashEquivalents", "Cash and cash equivalents", "Cash", "Cash & Cash Equivalents")
            ...
            debt_fcf_year = (debt_latest_val / fcf) if (np.isfinite(debt_latest_val) and np.isfinite(fcf) and fcf > 0) else np.nan
            ...
            with r1c1:
                st.metric(f"CFO ({_label_yr})", _fmt2(cfo, 0))
                st.metric("Cash Conversion (CFO/NP)", f"{_fmt2(cash_conv, 2)}%")
            with r1c2:
                st.metric(f"CapEx ({_label_yr})", _fmt2(capex, 0))
                st.metric("Debt / FCF (yrs)", _fmt2(debt_fcf_year, 2))
            with r1c3:
                st.metric(f"FCF ({_label_yr})", _fmt2(fcf, 0))
            ...
            with st.expander("Show calculation details — Cash Flow Wealth", expanded=False):
                ...
                st.markdown(
                    ...
                    f"- **FCF Yield ({_label_yr})** = FCF ÷ MarketCap = {(fcf if np.isfinite(fcf) else np.nan):,0f} ÷ {(market_cap_est if np.isfinite(market_cap_est) else np.nan):,0f} = **{(fcf_yield if np.isfinite(fcf_yield) else np.nan):,.2f}%**  \n"
                    f"- **Net Cash / MC** = NetCash ÷ MarketCap = {(net_cash if np.isfinite(net_cash) else np.nan):,0f} ÷ {(market_cap_est if np.isfinite(market_cap_est) else np.nan):,0f} = **{(netcash_mc if np.isfinite(netcash_mc) else np.nan):,.2f}%**  \n"
                    f"- **Interest Coverage** = EBITDA ÷ FinanceCosts = {(ebitda_fy if np.isfinite(ebitda_fy) else np.nan):,0f} ÷ {(fincost if np.isfinite(fincost) else np.nan):,0f} = **{(icov if np.isfinite(icov) else np.nan):,.2f}×**"
                )

                # Optional JSON download (FY)
                fy_payload = {
                    "stock": stock_name,
                    "type": "cashflow_wealth_fy",
                    "year": _latest_year,
                    "inputs": {
                        "CFO_FY": cfo, "CapEx_FY": capex, "Revenue_FY": rev, "NetProfit_FY": npf,
                        "EBITDA_FY": ebitda_fy, "FinanceCosts_FY": fincost, "Shares": shares,
                        "MarketCap_est": market_cap_est, "Debt_latest": debt_latest_val, "Cash_latest": cash_latest_val
                    },
                    "derived": {
                        "FCF_FY": fcf, "FCF_per_share_FY": fcf_ps, "FCF_margin_%": fcf_margin,
                        "FCF_yield_%": fcf_yield, "CashConversion_%": cash_conv, "NetCash": net_cash,
                        "NetCash_to_MC_%": netcash_mc, "Debt_to_FCF_years": debt_fcf_year,
                        "InterestCoverage": icov
                    }
                }
                _download_json_button("📥 Download Cash Flow Wealth (FY) JSON",
                                    fy_payload, f"{stock_name}_cashflow_fy.json",
                                    key=f"cashflow_fy_{stock_name}")
            # ----------------------------------------------------------------------
