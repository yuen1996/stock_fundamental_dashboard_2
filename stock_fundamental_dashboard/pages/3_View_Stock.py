...
# First, basic none/empty guard
if df is None or df.empty:
    st.warning("No data.")
    st.stop()

# Try normalize any weird casing to 'Name' BEFORE we guard on it
if "Name" not in df.columns:
    alt = next((c for c in df.columns if str(c).strip().lower() == "name"), None)
    if alt:
        df = df.rename(columns={alt: "Name"})

# Now, if still missing, stop cleanly
if "Name" not in df.columns:
    st.warning("No 'Name' column in data.")
    st.stop()
