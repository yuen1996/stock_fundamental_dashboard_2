    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("📥 Download — Annual Raw (JSON)", expanded=False):
                ...
                ttm_values = matrix_df[ttm_colname].to_dict() if ttm_colname and ttm_colname in matrix_df.columns else {}
            ...
            if ratio_df.empty:
                ...
                if ratio_layout.startswith("Years"):
                    ...
                    # ratio_df: Year-indexed (includes "TTM <year>" if created)
                    try:
                        ratio_year_rows = ratio_df.copy()
                    except Exception:
                        ratio_year_rows = pd.DataFrame()

                    try:
                        ratio_metric_rows = ratio_year_rows.T
                    except Exception:
                        ratio_metric_rows = pd.DataFrame()

                    ratio_payload = {
                        "stock": stock_name,
                        "industry": industry,
                        "industry_bucket": industry_bucket,
                        # whichever radio is selected: "Metrics → columns (Year rows)" or "Years → columns (Metric rows)"
                        "layout": ratio_layout,

                        # Canonical forms for downstream use
                        "year_rows": _records(ratio_year_rows),      # index = Year (e.g., 2019 … "TTM 2025")
                        "metric_rows": _records(ratio_metric_rows),  # index = Metric

                        # Components used to build TTM + the computed TTM ratios ({} if no TTM available)
                    ...
                    _download_json_button(
                        ...
                        key=f"annual_ratios_{stock_name}",
                    )
