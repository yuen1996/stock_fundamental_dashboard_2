...
def _download_json_button(label: str, obj: dict | list, filename: str) -> None:
    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — TTM", expanded=False):
                ...
                rev4 = _to_num(q.get(rev_col)).tail(4).sum() if rev_col else np.nan
                np4  = _to_num(q.get(np_col)).tail(4).sum() if np_col else np.nan
                eps4_sum = _to_num(q.get(eps_col)).tail(4).sum() if eps_col else np.nan
                sh4  = _to_num(q.get(sh_col)).tail(4) if sh_col else pd.Series(dtype="float64")
                sh_avg = float(sh4.dropna().mean()) if not sh4.dropna().empty else np.nan

                eps4_calc = eps4_sum
                if (not np.isfinite(eps4_sum) or float(eps4_sum) == 0.0) and np.isfinite(np4) and np.isfinite(sh_avg) and sh_avg > 0:
                    eps4_calc = float(np4) / float(sh_avg)  # fallback used by compute_ttm()

                pe_ttm = (cur_val / eps4_calc) if (np.isfinite(eps4_calc) and eps4_calc != 0) else np.nan

                st.markdown(
                    f"- **TTM Revenue** = sum(last 4 {rev_col or 'Revenue'}) = **{(rev4 if np.isfinite(rev4) else np.nan):,.0f}**  \n"
                    f"- **TTM Net Profit** = sum(last 4 {np_col or 'Net Profit'}) = **{(np4  if np.isfinite(np4)  else np.nan):,.0f}**  \n"
                    f"- **TTM EPS** = sum(last 4 {eps_col or 'EPS'})"
                    + ("" if (np.isfinite(eps4_sum) and float(eps4_sum) != 0.0) else " *(fallback: Net Profit ÷ avg Shares)*")
                    + f" = **{(eps4_calc if np.isfinite(eps4_calc) else np.nan):,.4f}**  \n"
                    f"- **P/E (TTM)** = Price ÷ TTM EPS = {cur_val:,.4f} ÷ { (eps4_calc if np.isfinite(eps4_calc) else np.nan):,.4f} "
                    f"= **{(pe_ttm if np.isfinite(pe_ttm) else np.nan):,.2f}**"
                )
                
                # --- P/B (TTM) display bits (robust even if MarketCap/Equity not stored) ---
                def _n(x):
                    try:
                        x = float(x)
                ...
                # MarketCap to display: prefer ttm["MarketCap"]; else approx via P/S × Revenue
                ps = _n((ttm or {}).get("P/S (TTM)"))
                rev4_num = _n(rev4)  # rev4 was computed above in this expander
                mc_disp = _n((ttm or {}).get("MarketCap"))
                if not np.isfinite(mc_disp) and np.isfinite(ps) and np.isfinite(rev4_num):
                    mc_disp = ps * rev4_num

                # Latest Equity to display: try common equity columns from the quarterly slice 'q'
                ...
                eq_disp = _n(q[eq_col].dropna().iloc[-1]) if (eq_col and not q.empty) else np.nan

                pb = _n((ttm or {}).get("P/B (TTM)"))

                st.markdown(
                    f"- **P/B (TTM)** = MarketCap ÷ Equity(latest) = "
                    f"{(mc_disp if np.isfinite(mc_disp) else np.nan):,.0f} ÷ "
                    f"{(eq_disp if np.isfinite(eq_disp) else np.nan):,.0f} "
                    f"= **{(pb if np.isfinite(pb) else np.nan):,.2f}**"
                )

                # ——— Download: TTM snapshot ———
                with st.expander("📄 Show calculation details — TTM", expanded=False):
                    # ttm already computed above: ttm = calculations.compute_ttm(stock, current_price=cur_val)
                    ttm_payload = {"stock": stock_name, "type": "ttm_snapshot", **ttm}
                    # (optional) small preview
                    st.write({k: ttm_payload[k] for k in list(ttm_payload.keys())[:6]})
            ...
            show_cf = st.checkbox("Show Cash Flow Wealth details", value=True, key=f"cfw_{stock_name}")
            if show_cf:
                ...
                with st.expander("Show calculation details — Cash Flow Wealth", expanded=False):
                    ...
                    cfw_payload = {
                        ...
                        "detail": {
                            "years": list(years_5y) if 'years_5y' in locals() else [],
                            "CFO":   list(cfo_5y)   if 'cfo_5y'   in locals() else [],
                            "CapEx": list(capex_5y) if 'capex_5y' in locals() else [],
                            "FCF":   list(fcf_5y)   if 'fcf_5y'   in locals() else [],
                            "CFO_pos_years": int(cfo_pos_yrs)   if 'cfo_pos_yrs'   in locals() else None,
                            "FCF_pos_years": int(fcf_pos_yrs)   if 'fcf_pos_yrs'   in locals() else None,
                            "cum_CFO": float(cum_cfo) if 'cum_cfo' in locals() else None,
                            "cum_NP":  float(cum_np)  if 'cum_np'  in locals() else None,
                            "cum_CFO/NP": float(cum_cov) if 'cum_cov' in locals() else None,
                        }
                    }
                    _download_json_button("📥 Download Cash-flow Wealth (JSON)",
                                        cfw_payload,
                                        f"{stock_name}_cashflow_wealth.json",
                                        key=f"cfw_{stock_name}")
            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                ...
                ratio_json = {
                    "stock": stock_name,
                    "type": "annual_ratios",
                    "ratios_by_year": ratio_df.reset_index().replace({pd.NA: None}).where(
                        pd.notna(ratio_df.reset_index()), None
                    ).to_dict(orient="records"),
                }
                ...
                _download_json_button("📥 Download This Year's Raw Row (JSON)",
                                    raw_ann_json, f"{stock_name}_{sel_year}_annual_raw.json")
...
with tabs[1]:
    ...
    with st.expander("Download — Quarterly Raw (JSON)", expanded=False):
        q_payload = {
            ...
            "visible_table": (disp_qnum.reset_index().to_dict(orient="records") if isinstance(disp_qnum, pd.DataFrame) else []),
            "raw_rows": quarterly.replace({pd.NA: None}).to_dict(orient="records"),
        }
        _download_json_button("📥 Download Quarterly Raw (JSON)", q_payload, f"{stock_name}_quarterly_raw.json")
