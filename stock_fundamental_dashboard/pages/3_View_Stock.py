    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            for _, row in annual.iterrows():
                ...
                # Depreciation & Amortization (sum splits if present)
                dep_total = (
                    sum(x for x in [
                        _n(rowx.get("DepPPE")),
                        _n(rowx.get("DepInvProp")),
                        _n(rowx.get("DepROU")),
                    ] if np.isfinite(x))
                    if any([rowx.get("DepPPE"), rowx.get("DepInvProp"), rowx.get("DepROU")])
                    else np.nan
                )

                # --- EBITDA (robust derivation)
                ebitda_val = _n(rowx.get("EBITDA"))  # 1) raw if present

                if not np.isfinite(ebitda_val):
                    # 2) EBIT + D&A
                    op = _n(rowx.get("OperatingProfit"))
                    if not np.isfinite(op):
                        gp  = _n(rowx.get("GrossProfit"))
                        adm = _n(rowx.get("AdminExpenses"))
                        sde = _n(rowx.get("SellDistExpenses"))
                        if all(np.isfinite(x) for x in [gp, adm, sde]):
                            op = gp - adm - sde
                    if np.isfinite(op) and np.isfinite(dep_total):
                        ebitda_val = op + dep_total

                if not np.isfinite(ebitda_val):
                    # 3) NP + Finance + Tax + D&A (last-resort)
                    npf = _n(rowx.get("NetProfit"))
                    fin = _n(rowx.get("FinanceCosts"))
                    tax = _n(rowx.get("IncomeTax"))
                    parts = [npf, fin, tax, dep_total]
                    if any(np.isfinite(p) for p in parts):
                        ebitda_val = float(np.nansum([p for p in parts if np.isfinite(p)]))
