    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            for _, row in annual.iterrows():
                ...
                total_debt = (
                    (_n(rowx.get("Borrowings")) if np.isfinite(_n(rowx.get("Borrowings"))) else 0.0) +
                    (_n(rowx.get("OtherLoans")) if np.isfinite(_n(rowx.get("OtherLoans"))) else 0.0)
                )
                ...
                r.update({
                    ...
                    "EBITDA":                      _n(rowx.get("EBITDA")),   # let this be raw or derived upstream
                    # show blank when there’s truly no debt (keeps table clean)
                    "Total Debt / Borrowings":     (np.nan if total_debt == 0 else total_debt),
                })
            ...
            try:
                ...
                if not q4.empty:
                    ...
                    # --- ADD: same convenience fields for TTM
                    r_ttm.update({
                        "CapEx":                        synth.get("CapEx"),
                        "Operating Cash Flow":          synth.get("CFO"),
                        # no quarterly split for D&A here; fall back to DepAmort if you have it in SUM_MAP
                        "Depreciation & Amortization":  synth.get("DepAmort"),
                        "EBITDA":                       synth.get("EBITDA"),
                        # Total debt (TTM) isn’t a sum; leave blank unless you start carrying quarterly debt
                        "Total Debt / Borrowings":      np.nan,
                    })
