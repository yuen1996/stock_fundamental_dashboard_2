    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — Cash Flow Wealth", expanded=False):
                ...
                fincost = n((ttm or {}).get("Finance Costs (TTM)"))  # only if present
            ...
            with st.expander("Show calculation details — Snowflake", expanded=False):
                ...
                if detail:
                    ...
                    # component drill-down (HTML <details>) — keep as-is
                    ...
                    _download_json_button("📥 Download Snowflake Detail (JSON)",
                                        sf_payload,
            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                sel_year = int(years[-1]) if len(years) else None
                if sel_year is None:
                    st.info("No annual history.")
                else:
                    st.markdown(f"**Year**\n\n{sel_year}")

                    # pull row (safe) for formulas
                    raw = {}
                    try:
                        raw = {
                            "Revenue":       float(ann_numeric.loc[("Income Statement","Revenue"),        str(sel_year)]),
                            "NetProfit":     float(ann_numeric.loc[("Income Statement","Net Profit"),     str(sel_year)]),
                            "CostOfSales":   float(ann_numeric.loc[("Income Statement","Cost Of Sales"),  str(sel_year)]),
                            "FinanceCosts":  float(ann_numeric.loc[("Income Statement","Finance Costs"),  str(sel_year)]),
                            "AdminExpenses": float(ann_numeric.loc[("Income Statement","Administrative Expenses"), str(sel_year)]),
                            "SellDistExpenses": float(ann_numeric.loc[("Income Statement","Selling & Distribution Expenses"), str(sel_year)]),
                            "NumShares":     float(ann_numeric.loc[("Balance Sheet","Number of Shares"),  str(sel_year)]),
                            "CurrentAsset":  float(ann_numeric.loc[("Balance Sheet","Current Asset"),     str(sel_year)]),
                            "CurrentLiability": float(ann_numeric.loc[("Balance Sheet","Current Liabilities"), str(sel_year)]),
                            "Inventories":   float(ann_numeric.loc[("Balance Sheet","Inventories"),       str(sel_year)]),
                            "TotalAsset":    float(ann_numeric.loc[("Balance Sheet","Total Asset"),       str(sel_year)]),
                            "TotalLiability":float(ann_numeric.loc[("Balance Sheet","Total Liability"),   str(sel_year)]),
                            "ShareholderEquity": float(ann_numeric.loc[("Balance Sheet","Shareholder Equity"), str(sel_year)]),
                            "Dividend":      float(ann_numeric.loc[("Share Data","Dividend"),             str(sel_year)]) if ("Share Data","Dividend") in ann_numeric.index else 0.0,
                            "SharePrice":    float(ann_numeric.loc[("Share Data","Share Price"),          str(sel_year)]) if ("Share Data","Share Price") in ann_numeric.index else float(cur_val or 0),
                        }
                    except Exception:
                        pass
                    ...
                    def p(a,b):
                        try:
                            a=n(a); b=n(b)
                            return None if (a is None or b is None or b==0) else a/b*100.0
                        except: return None

                    # metrics (same as the table)
                    eps   = (n(raw.get("NetProfit")) / n(raw.get("NumShares"))) if n(raw.get("NetProfit")) and n(raw.get("NumShares")) else None
                    bvps  = (n(raw.get("ShareholderEquity")) / n(raw.get("NumShares"))) if n(raw.get("ShareholderEquity")) and n(raw.get("NumShares")) else None
                    roe   = p(raw.get("NetProfit"), raw.get("ShareholderEquity"))
                    gm    = p(n(raw.get("Revenue"))-n(raw.get("CostOfSales")), raw.get("Revenue"))
                    nm    = p(raw.get("NetProfit"), raw.get("Revenue"))
                    debtA = p(raw.get("TotalLiability"), raw.get("TotalAsset"))
                    curR  = (n(raw.get("CurrentAsset"))/n(raw.get("CurrentLiability"))) if n(raw.get("CurrentLiability")) else None
                    quickR= ((n(raw.get("CurrentAsset"))-n(raw.get("Inventories")))/n(raw.get("CurrentLiability"))) if n(raw.get("CurrentLiability")) else None
                    pe    = (n(raw.get("SharePrice"))/n(eps)) if n(raw.get("SharePrice")) and n(eps) else None
                    pb    = (n(raw.get("SharePrice"))/n(bvps)) if n(raw.get("SharePrice")) and n(bvps) else None
                    yld   = p(raw.get("Dividend"), raw.get("SharePrice"))

                    st.markdown(
                        "\n".join([
                            f"- **EPS** = Net Profit ÷ Shares = { _fmt_num(raw.get('NetProfit'), d=0) } ÷ { _fmt_num(raw.get('NumShares'), d=0) } = **{_fmt_num(eps, d=4)}**",
                            f"- **ROE** = Net Profit ÷ Equity × 100 = {_fmt_num(raw.get('NetProfit'), d=0)} ÷ {_fmt_num(raw.get('ShareholderEquity'), d=0)} = **{_fmt_pct(roe)}**",
                            f"- **P/E** = Price ÷ EPS = {_fmt_num(raw.get('SharePrice'), d=4)} ÷ {_fmt_num(eps, d=4)} = **{_fmt_num(pe, d=2, commas=False)}**",
                            f"- **P/B** = Price ÷ BVPS = {_fmt_num(raw.get('SharePrice'), d=4)} ÷ {_fmt_num(bvps, d=4)} = **{_fmt_num(pb, d=2, commas=False)}**",
                            f"- **Gross Margin** = (Revenue − COGS) ÷ Revenue × 100 = **{_fmt_pct(gm)}**",
                            f"- **Net Margin** = Net Profit ÷ Revenue × 100 = **{_fmt_pct(nm)}**",
                            f"- **Current Ratio** = Current Assets ÷ Current Liabilities = **{_fmt_num(curR, d=2, commas=False)}×**",
                            f"- **Quick Ratio** = (CA − Inventories) ÷ CL = **{_fmt_num(quickR, d=2, commas=False)}×**",
                            f"- **Dividend Yield** = Dividend ÷ Price × 100 = **{_fmt_pct(yld)}**",
                            f"- **BVPS** = Equity ÷ Shares = **{_fmt_num(bvps, d=4)}**",
                        ])
                    )
                    ...
                    ratio_json = {
                        "stock": stock_name,
                        "type": "annual_ratios",
                        "ratios_by_year": ratio_df.reset_index().to_dict(orient="records"),
                    }
                    _download_json_button(
                        "📥 Download Annual Ratios (JSON)",
                        ratio_json, f"{stock_name}_annual_ratios.json",
                        key=f"annual_ratios_{stock_name}_{sel_year}"
                    )
                    ...
                    _download_json_button(
                        "📥 Download Selected Year Raw Row (JSON)",
                        raw_ann_json, f"{stock_name}_annual_raw_{sel_year}.json",
                        key=f"annual_rawrow_{stock_name}_{sel_year}"
                    )
