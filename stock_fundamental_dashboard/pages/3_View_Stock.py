            ...
            with st.expander("Show calculation details — Annual ratios", expanded=False):
                ...
                if not year_labels:
                    ...
                    if ttm_label and sel_label == ttm_label:
                        ...
                        # --- NEW: CapEx, CFO, D&A, EBITDA, Debt (for the selected YEAR) ---

                        # Direct values
                        cfo_y   = n(raw.get("CFO"))
                        capex_y = n(raw.get("CapEx"))

                        # D&A from splits or DepAmort
                        da_y = _dep_total_from_row(raw)
                        _da_parts = []
                        for lab, key in [("Dep PPE", "DepPPE"), ("Dep InvProp", "DepInvProp"), ("Dep ROU", "DepROU")]:
                            v = n(raw.get(key))
                            if np.isfinite(v):
                                _da_parts.append(f"{lab}={v:,.0f}")
                        if not _da_parts:
                            v = n(raw.get("DepAmort"))
                            if np.isfinite(v):
                                _da_parts.append(f"DepAmort={v:,.0f}")
                        da_parts_str_y = " + ".join(_da_parts) if _da_parts else "–"

                        # EBITDA (reported → EBIT+D&A → NP+Fin+Tax+D&A)
                        ...
                        if np.isfinite(raw_ebitda_y) and raw_ebitda_y != 0:
                            ebitda_expl_y = f"EBITDA (reported) = **{raw_ebitda_y:,.0f}**"
                        elif np.isfinite(opy) and np.isfinite(da_y):
                            ebitda_expl_y = f"EBITDA = EBIT + D&A = ({ebit_src_y}: {opy:,.0f}) + ({da_y:,.0f}) = **{(opy + da_y):,.0f}**"
                        else:
                            ...
                            shown = [f"{k}={v:,.0f}" for k, v in parts if np.isfinite(v)]
                            total = float(np.nansum([v for _, v in parts if np.isfinite(v)])) if shown else np.nan
                            ebitda_expl_y = f"EBITDA = " + " + ".join(shown) + f" = **{total:,.0f}**" if shown else "EBITDA = –"
                        ...
                        debt_shown_y = [f"{k}={v:,.0f}" for k, v in debt_parts_y if np.isfinite(v)]
                        debt_sum_y   = float(np.nansum([v for _, v in debt_parts_y if np.isfinite(v)])) if debt_shown_y else np.nan

                        st.markdown(
                            f"- **CapEx** = **{(capex_y if np.isfinite(capex_y) else np.nan):,.0f}**  \n"
                            f"- **Operating Cash Flow** = **{(cfo_y if np.isfinite(cfo_y) else np.nan):,.0f}**  \n"
                            f"- **Depreciation & Amortization** = {da_parts_str_y} = **{(da_y if np.isfinite(da_y) else np.nan):,.0f}**  \n"
                            f"- {ebitda_expl_y}  \n"
                            f"- **Total Debt / Borrowings** = "
                            + ((" + ".join(debt_shown_y) + f" = **{debt_sum_y:,.0f}**") if debt_shown_y else "–")
                        )
                    ...
                    # ----- normal annual-year branch (your existing calculations) -----
                        ...
                        # --- NEW: CapEx, CFO, D&A, EBITDA, Debt (TTM) with “what + what” ---

                        # Values from the TTM synthetic row we built earlier
                        cfo_ttm   = n(comp.get("CFO"))
                        capex_ttm = n(comp.get("CapEx"))

                        # Depreciation & Amortization (sum splits or DepAmort fallback)
                        da_ttm = _dep_total_from_row(comp)

                        # Build a readable “parts” string for D&A
                        _da_parts = []
                        for lab, key in [("Dep PPE", "DepPPE"), ("Dep InvProp", "DepInvProp"), ("Dep ROU", "DepROU")]:
                            v = n(comp.get(key))
                            if np.isfinite(v):
                        ...
                        if not _da_parts:
                            ...
                            if np.isfinite(v):
                                _da_parts.append(f"DepAmort={v:,.0f}")
                        da_parts_str = " + ".join(_da_parts) if _da_parts else "–"

                        # EBITDA path (reported → EBIT+D&A → NP+Fin+Tax+D&A)
                        raw_ebitda = n((ttm_detail_export or {}).get("ratios", {}).get("EBITDA"))
                        ...
                        if np.isfinite(raw_ebitda) and raw_ebitda != 0:
                            ebitda_expl = f"EBITDA (reported) = **{raw_ebitda:,.0f}**"
                        elif np.isfinite(op) and np.isfinite(da_ttm):
                            ebitda_expl = f"EBITDA = EBIT + D&A = ({ebit_src}: {op:,.0f}) + ({da_ttm:,.0f}) = **{(op + da_ttm):,.0f}**"
                        else:
                            ...
                            shown = [f"{k}={v:,.0f}" for k, v in parts if np.isfinite(v)]
                            total = float(np.nansum([v for _, v in parts if np.isfinite(v)])) if shown else np.nan
                            ebitda_expl = f"EBITDA = " + " + ".join(shown) + f" = **{total:,.0f}**" if shown else "EBITDA = –"

                        # Total Debt / Borrowings (latest quarterly snapshot)
                        def _last_q(col):
                        ...
                        debt_shown = [f"{k}={v:,.0f}" for k, v in debt_parts if np.isfinite(v)]
                        debt_sum   = float(np.nansum([v for _, v in debt_parts if np.isfinite(v)])) if debt_shown else np.nan

                        st.markdown(
                            f"- **CapEx (TTM)** = **{(capex_ttm if np.isfinite(capex_ttm) else np.nan):,.0f}**  \n"
                            f"- **Operating Cash Flow (TTM)** = **{(cfo_ttm if np.isfinite(cfo_ttm) else np.nan):,.0f}**  \n"
                            f"- **Depreciation & Amortization (TTM)** = {da_parts_str} = **{(da_ttm if np.isfinite(da_ttm) else np.nan):,.0f}**  \n"
                            f"- {ebitda_expl}  \n"
                            f"- **Total Debt / Borrowings (latest)** = "
                            + ((" + ".join(debt_shown) + f" = **{debt_sum:,.0f}**") if debt_shown else "–")
                        )
