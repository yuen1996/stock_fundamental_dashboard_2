...
with tabs[1]:
    ...
    if not qratio_df.empty:
        with st.expander("Show calculation details — Quarterly ratios", expanded=False):
            ...
            sel_per = st.selectbox("Period", options=per_opts, index=sel_idx, key=f"q_ratio_detail_per_{stock_name}")

            # Parse "YYYY QN"
            year_num, q_num = None, None
            m = re.match(r"^\\s*(\\d{4})\\s+Q(\\d)\\s*$", str(sel_per))
            if m:
                year_num = int(m.group(1)); q_num = int(m.group(2))

            # Get raw quarterly row matching that period
            raw_q = quarterly.copy()
            if "Quarter" in raw_q.columns:
                raw_q["Qnum"] = raw_q["Quarter"].map(quarter_key_to_num)
            if year_num is not None and q_num is not None:
                raw_q = raw_q[(raw_q.get("Year") == year_num) & (raw_q.get("Qnum") == q_num)]
            raw_rec_q = raw_q.tail(1).to_dict("records")
            rawq = raw_rec_q[0] if raw_rec_q else {}
            ...
            dy    = (div_ps / price * 100.0) if (np.isfinite(div_ps) and np.isfinite(price) and price != 0) else np.nan
            bvps  = (eq / sh) if (np.isfinite(eq) and np.isfinite(sh) and sh != 0) else np.nan
            nta   = ((eq - n(rawq.get("Q_IntangibleAsset"))) / sh) if (np.isfinite(eq) and np.isfinite(sh) and sh != 0 and np.isfinite(n(rawq.get("Q_IntangibleAsset")))) else np.nan
            ...
            st.markdown(
                ...
                f"- **Dividend Yield** ≈ Dividend/share ÷ Price = {div_ps:,.4f} ÷ {price:,.4f} = **{dy:,.2f}%**  \n"
                f"- **BVPS** = Equity ÷ Shares = {eq:,.0f} ÷ {sh:,.0f} = **{bvps:,.4f}**  \n"
                + (f"- **NTA / share** = (Equity − Intangibles) ÷ Shares = {(eq - n(rawq.get('Q_IntangibleAsset'))):,.0f} ÷ {sh:,.0f} = **{nta:,.4f}**"
                if np.isfinite(n(rawq.get("Q_IntangibleAsset"))) else "")
            )
