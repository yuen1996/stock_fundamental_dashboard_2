    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — TTM", expanded=False):
                # show the recent quarters table (latest first, TTM uses last 4)
                q = quarterly.copy()
                q["_Q"] = q["Quarter"].apply(quarter_key_to_num)
                q = q.dropna(subset=["Year", "_Q"]).sort_values(["Year", "_Q"])
                show_cols = ["Year", "Quarter", "Q_Revenue", "Q_NetProfit", "Q_NumShares", "Q_EndQuarterPrice"]
                show_cols = [c for c in show_cols if c in q.columns]
                st.dataframe(q[show_cols].tail(10), use_container_width=True, height=280)
                ...
                def _latest(df, candidates):
                    ...
                    return float("nan")
                def _fmt(x, p=0, pct=False):
                    if x is None or (isinstance(x, float) and (np.isnan(x) or np.isinf(x))):
                        return "–"
                    return (f"{x*100:.2f}%" if pct else f"{x:,.{p}f}")

                # inputs from compute_ttm()
                cur_price = ttm.get("Current Price", cur_val if "cur_val" in locals() else None)
                shares    = ttm.get("Shares")
                mc        = ttm.get("MarketCap")
                rev_ttm   = ttm.get("TTM Revenue")
                gp_ttm    = ttm.get("TTM Gross Profit")
                op_ttm    = ttm.get("TTM Operating Profit")
                np_ttm    = ttm.get("TTM Net Profit")
                eps_ttm   = ttm.get("TTM EPS")
                ebitda_ttm= ttm.get("TTM EBITDA")

                # other primitives for valuation
                equity_latest = _latest(stock, [
                    "ShareholderEquity","Shareholder Equity","Equity",
                    "TotalEquity","Total Equity","Q_ShareholderEquity"
                ])
                debt_latest   = _latest(stock, ["TotalDebt","Debt","Borrowings"])
                cash_latest   = _latest(stock, ["Cash","CashAndEquivalents","Cash & Equivalents"])

                # derived
                gross_m   = (gp_ttm / rev_ttm) if rev_ttm else float("nan")
                oper_m    = (op_ttm / rev_ttm) if rev_ttm else float("nan")
                net_m     = (np_ttm / rev_ttm) if rev_ttm else float("nan")
                pe_ttm    = (cur_price / eps_ttm) if (cur_price and eps_ttm) else float("nan")
                ps_ttm    = (mc / rev_ttm) if (mc and rev_ttm) else float("nan")
                pb_ttm    = (mc / equity_latest) if (mc and equity_latest) else float("nan")
                net_debt  = (debt_latest or 0.0) - (cash_latest or 0.0)
                ev_ebitda = ((mc + net_debt) / ebitda_ttm) if (mc and ebitda_ttm) else float("nan")

                st.markdown(
                    "- **TTM Revenue** = sum(last 4 Q_Revenue) = **" + _fmt(rev_ttm, 0) + "**  \n"
                    "- **TTM Gross Margin** = GrossProfit_TTM ÷ Revenue_TTM = "
                    f"{_fmt(gp_ttm,0)} ÷ {_fmt(rev_ttm,0)} = **{_fmt(gross_m,2,True)}**  \n"
                    "- **TTM Operating Margin** = OperatingProfit_TTM ÷ Revenue_TTM = "
                    f"{_fmt(op_ttm,0)} ÷ {_fmt(rev_ttm,0)} = **{_fmt(oper_m,2,True)}**  \n"
                    "- **TTM Net Profit** = sum(last 4 Q_NetProfit) = **" + _fmt(np_ttm, 0) + "**  \n"
                    "- **TTM Net Margin** = NetProfit_TTM ÷ Revenue_TTM = "
                    f"{_fmt(np_ttm,0)} ÷ {_fmt(rev_ttm,0)} = **{_fmt(net_m,2,True)}**  \n"
                    "- **TTM EPS** = sum(last 4 Q_EPS)"
                    " (fallback: NetProfit_TTM ÷ latest Shares) = **" + _fmt(eps_ttm, 4) + "**  \n"
                    "- **P/E (TTM)** = Price ÷ EPS_TTM = "
                    f"{_fmt(cur_price,4)} ÷ {_fmt(eps_ttm,4)} = **{_fmt(pe_ttm,2)}**  \n"
                    "- **P/S (TTM)** = MarketCap ÷ Revenue_TTM = "
                    f"{_fmt(mc,0)} ÷ {_fmt(rev_ttm,0)} = **{_fmt(ps_ttm,2)}**  \n"
                    "- **P/B (TTM)** = MarketCap ÷ Equity(latest) = "
                    f"{_fmt(mc,0)} ÷ {_fmt(equity_latest,0)} = **{_fmt(pb_ttm,2)}**  \n"
                    "- **EV/EBITDA (TTM)** = (MarketCap + NetDebt) ÷ EBITDA_TTM = "
                    f"({_fmt(mc,0)} + {_fmt(net_debt,0)}) ÷ {_fmt(ebitda_ttm,0)} = **{_fmt(ev_ebitda,2)}**  \n"
                    "- **Inputs** → Price: " + _fmt(cur_price,4) + " • Shares: " + _fmt(shares,0) +
                    " • MarketCap: " + _fmt(mc,0) + " • Equity(latest): " + _fmt(equity_latest,0) +
                    " • Debt(latest): " + _fmt(debt_latest,0) + " • Cash(latest): " + _fmt(cash_latest,0)
                )

                # download – everything we just used
                payload = {
                    "stock": stock_name,
                    "type": "ttm_detail",
                    "inputs": {
                        "Price": cur_price, "Shares": shares, "MarketCap": mc,
                        "Equity_latest": equity_latest, "Debt_latest": debt_latest, "Cash_latest": cash_latest
                    },
                    "ttm": {
                    ...
                    "ratios": {
                        "GrossMargin_%": None if not rev_ttm else 100*gp_ttm/rev_ttm,
                        "OperatingMargin_%": None if not rev_ttm else 100*op_ttm/rev_ttm,
                        "NetMargin_%": None if not rev_ttm else 100*np_ttm/rev_ttm,
                        "PE_TTM": pe_ttm, "PS_TTM": ps_ttm, "PB_TTM": pb_ttm, "EV_EBITDA_TTM": ev_ebitda
                    }
                }
                _download_json_button("📥 Download TTM Snapshot (JSON)", payload,
                                    f"{stock_name}_ttm_detail.json", key=f"ttm_detail_{stock_name}")
