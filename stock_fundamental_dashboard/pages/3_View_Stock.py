...
st.markdown(
    ...
    '<div class="d">Find stocks by name and industry; optionally require annual/quarterly data</div></div>',
    unsafe_allow_html=True
)

# Robust: handle missing Industry
...
with c3:
    only_ann = st.checkbox("Only with Annual data", value=False, key="vs_only_ann")
with c4:
    only_qtr = st.checkbox("Only with Quarterly data", value=False, key="vs_only_qtr")
...
# “has annual/quarterly” filters act at the stock level
if only_ann:
    ann_names = set(df_f.loc[df_f.get("IsQuarter", False) != True, "Name"].dropna().astype(str).unique().tolist())
    df_f = df_f[df_f["Name"].astype(str).isin(ann_names)]
if only_qtr:
    qtr_names = set(df_f.loc[df_f.get("IsQuarter", False) == True, "Name"].dropna().astype(str).unique().tolist())
    df_f = df_f[df_f["Name"].astype(str).isin(qtr_names)]

# Final list used by the expanders below
stocks = sorted([s for s in df_f["Name"].dropna().astype(str).unique().tolist()])

st.caption(f"Showing **{len(stocks)}** stock(s){' in '+pick_ind if pick_ind!='All' else ''}{' matching “'+q_text+'”' if q_text else ''}.")
