    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — TTM", expanded=False):
                ...
                # ---------- JSON payload (include the extra TTM items) ----------
                ttm_json = {
                    "window": "last 4 quarters",
                    "inputs": {
                        "price": price_ttm,
                        "shares_last_q": shares_last_q,
                        "shares_avg_ttm": shares_av,
                        "market_cap_ttm": mc_ttm,
                        "equity_latest": eq_ttm,
                    ...
                    "ttm_additional": {
                        "cash": cash_ttm,
                        "current_lease_liab": lease_cur_ttm,
                        "noncurrent_lease_liab": lease_ncur_ttm,
                        "cfo": cfo_ttm,
                        "capex": capex_ttm,
                        "income_tax": tax_ttm,
                        "dep_ppe": depppe_ttm,
                        "dep_inv_property": depinv_ttm,
                        "dep_rou": deprou_ttm,
                        "borrowings": borrow_ttm,
                        "other_loans": otherloan_ttm
                    }
                }
                _download_json_button("📥 Download Annual Raw (JSON)", ttm_json,
                                    f"{stock_name}_annual_raw_ttm.json",
                                    key=f"annual_raw_ttm_{stock_name}")
