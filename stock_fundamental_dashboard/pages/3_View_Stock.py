        ...
        with tabs[0]:
            ...
            with st.expander("Show calculation details — TTM", expanded=False):
                ...
                cols_cand = {
                    ...
                    "DebtQ":           ["Q_TotalDebt","TotalDebt","Debt","Borrowings"],
            ...
            with st.expander("Show calculation details — TTM", expanded=False):
                # Show last 8 quarters & the exact columns used
                cols_found = {
                    "Revenue from": [c for c in rev_cols if c in stock.columns],
                    "GrossProfit from": [c for c in gp_cols if c in stock.columns],
                    "OperatingProfit from": [c for c in op_cols if c in stock.columns],
                    "NetProfit from": [c for c in np_cols if c in stock.columns],
                    "EPS from": [c for c in eps_cols if c in stock.columns],
                    "Shares from": [c for c in sh_cols if c in stock.columns],
                    "Price from": [c for c in px_cols if c in stock.columns],
                    "EBITDA from": [c for c in ebitda_cols if c in stock.columns],
                    "Equity from": [c for c in eq_cols if c in stock.columns],
                }
                st.write("Columns detected per metric:", cols_found)
                ...
                # — Download: TTM snapshot (keeps your existing utility) —
                ttm_payload = {
                    "stock": stock_name,
                    "type": "ttm_snapshot",
                    "inputs": {
                        "Price": price_now, "Shares_avg4Q": (float(SH4.mean()) if not SH4.empty else None),
                        "MarketCap": market_cap, "Equity_latest": (equity_latest if 'equity_latest' in locals() else None),
                        "Debt_latest": debt_latest, "Cash_latest": cash_latest
                    },
                    "ttm": {
            ...
            try:
                ...
                if not q4.empty:
                    ...
                    # Always prefer the live/current price for TTM calculations
                    try:
                        if cur_val is not None and np.isfinite(cur_val):
