    ...
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            if st.checkbox("TTM deep debug", key=f"ttm_deep_dbg_{stock_name}"):
                ...
                # Not enough to compute TTM totals — count how many usable recent quarters we have
                n_q = 0
                if not quarterly.empty:
                    qh = quarterly.copy()
                    qh["Q_Revenue"]   = pd.to_numeric(qh.get("Q_Revenue"), errors="coerce")
                    qh["Q_NetProfit"] = pd.to_numeric(qh.get("Q_NetProfit"), errors="coerce")
                    qh["Q_EPS"]       = pd.to_numeric(qh.get("Q_EPS"), errors="coerce")
                    qh["has"] = qh[["Q_Revenue", "Q_NetProfit", "Q_EPS"]].any(axis=1)
                    # recent quarters with any usable value
                    n_q = int(qh["has"].tail(8).sum())
                st.info(
                    f"Not enough data for TTM — need the latest 4 quarters with numbers. "
                    f"Found {n_q} quarter(s)."
                )
            ...
            # Cross-check cash-flow inputs that drive these cards
            cf_need = ["TTM CFO", "TTM CapEx", "TTM FCF", "Net Cash (Debt)", "Interest Coverage (EBITDA/Fin)"]
            cf_missing = [k for k in cf_need if not _isnum((ttm or {}).get(k))]
            if cf_missing:
                st.info("Not enough data for some cash-flow metrics: " + ", ".join(cf_missing))
            ...
            # Snowflake relies on TTM totals; warn if core pieces are missing
            core_ttm_need = ["TTM Revenue", "TTM Net Profit", "TTM EPS"]
            core_missing = [k for k in core_ttm_need if not _isnum((ttm or {}).get(k))]
            if core_missing:
                st.warning("Snowflake/Cash metrics may be partial — missing: " + ", ".join(core_missing))
            ...
            if st.checkbox("ℹ️  Show Snowflake calculation detail", key=f"sf_detail_{stock_name}"):
	    ...
	    # Cross-check inputs for latest year ratios
	    if not ratio_df.empty:
              latest_y = int(ratio_df.index.max())
                need_cols = [
                "ROE (%)", "P/E", "P/B",
                "Gross Profit Margin (%)", "Net Profit Margin (%)",
                "Current Ratio", "Quick Ratio", "Dividend Yield (%)"
            ]
            missing = [c for c in need_cols if c not in ratio_df.columns or pd.isna(ratio_df.loc[latest_y].get(c))]
            if missing:
                st.info("Not enough data to compute some ratios for the latest year: " + ", ".join(missing))
