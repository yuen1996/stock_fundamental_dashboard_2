    ...
    if qratio_df.empty:
        ...
        if not qratio_df.empty:
            with st.expander("Show calculation details — Quarterly ratios", expanded=False):
                ...
                # 1) Selected period only
                _sel_row = qratio_df.loc[sel_per].copy()
                try:
                    _sel_row = _sel_row.astype(float)
                except Exception:
                    pass

                sel_payload = {
                    "stock":  stock_name,
                    "type":   "quarterly_ratio_detail",
                    "period": str(sel_per),
                    "ratios": {
                        k: (None if pd.isna(v) or (isinstance(v, float) and np.isinf(v)) else float(v))
                        for k, v in _sel_row.to_dict().items()
                    },
                }
                _download_json_button(
                    "📥 Download Selected Period Ratios (JSON)",
                    sel_payload,
                    f"{stock_name}_{str(sel_per).replace(' ', '_')}_ratios.json",
                    key=f"dl_qr_sel_{stock_name}"
                )
