...
for stock_name in stocks:
    with st.expander(stock_name, expanded=False):
        ...
        with tabs[0]:
            ...
            try:
                ...
                if not q4.empty:
                    ...
                    LAST_MAP = {
                        ...
                        "Reserves": "Q_Reserves",
                        # prices (if present in your data)
            ...
            with st.expander("Show calculation of TTM (to …)", expanded=False):
                # --- build a consistent visible matrix order for the JSON (same as before)
                try:
                ...
                # --- pull the last few quarters and show the exact inputs used
                ...
                # pick the columns like your TTM panel does
                cols_cand = {
                    ...
                    "Debt": ["Q_TotalDebt","TotalDebt","Debt","Borrowings"],
                    "Cash": ["Q_Cash","Cash","CashAndEquivalents","Cash & Cash Equivalents","Cash & Equivalents"],
                    "EBITDA": ["Q_EBITDA","EBITDA","Quarterly EBITDA"],
                }
                ...
                debt_c  = pick(cols_cand["Debt"])
                cash_c  = pick(cols_cand["Cash"])
                ebd_c   = pick(cols_cand["EBITDA"])
                ...
                # --- explicit TTM calculations (match Photo #1 wording)
                ...
                price_latest = _last(q.get(pr_col)) if pr_col else (cur_val if _np.isfinite(cur_val) else _np.nan)
                shares_latest = _last(q.get(sh_col)) if sh_col else _np.nan
                market_cap = (price_latest * shares_latest) if (_np.isfinite(price_latest) and _np.isfinite(shares_latest)) else _np.nan

                equity_latest = _last(q.get(eq_col)) if eq_col else _np.nan
                debt_latest   = _last(q.get(debt_c)) if debt_c else _np.nan
                cash_latest   = _last(q.get(cash_c)) if cash_c else _np.nan
                net_debt = (debt_latest if _np.isfinite(debt_latest) else 0.0) - (cash_latest if _np.isfinite(cash_latest) else 0.0)

                ebitda_ttm = _to_num(q.get(ebd_c)).tail(4).sum() if ebd_c else _np.nan
                ...
                pe_ttm = (price_latest / eps4_calc) if (_np.isfinite(price_latest) and _np.isfinite(eps4_calc) and eps4_calc != 0) else _np.nan
                ...
                ev_ebitda = ((market_cap + net_debt) / ebitda_ttm) if (_np.isfinite(market_cap) and _np.isfinite(ebitda_ttm) and ebitda_ttm != 0) else _np.nan

                # update expander title with the actual year
                if last_year:
                    st.write("")  # force a small reflow
                ...
                st.markdown(
                    ...
                    f"{'' if (_np.isfinite(eps4_sum) and float(eps4_sum) != 0.0) else ' *(fallback: Net Profit ÷ avg Shares)*'}"
                    f" = **{(eps4_calc if _np.isfinite(eps4_calc) else _np.nan):,.4f}**  \n"
                    f"- **P/E (TTM)** = Price ÷ EPS_TTM = {price_latest:,.4f} ÷ {(eps4_calc if _np.isfinite(eps4_calc) else _np.nan):,.4f} = **{(pe_ttm if _np.isfinite(pe_ttm) else _np.nan):,.2f}**  \n"
                    f"- **P/S (TTM)** = MarketCap ÷ Revenue_TTM = {(market_cap if _np.isfinite(market_cap) else _np.nan):,.0f} ÷ {(rev4 if _np.isfinite(rev4) else _np.nan):,.0f} = **{(ps_ttm if _np.isfinite(ps_ttm) else _np.nan):,.2f}**  \n"
                    f"- **P/B (TTM)** = MarketCap ÷ Equity(latest) = {(market_cap if _np.isfinite(market_cap) else _np.nan):,.0f} ÷ {(equity_latest if _np.isfinite(equity_latest) else _np.nan):,.0f} = **{(pb_ttm if _np.isfinite(pb_ttm) else _np.nan):,.2f}**  \n"
                    f"- **EV/EBITDA (TTM)** = (MarketCap + NetDebt) ÷ EBITDA_TTM = ({(market_cap if _np.isfinite(market_cap) else _np.nan):,.0f} + {(net_debt if _np.isfinite(net_debt) else _np.nan):,.0f}) ÷ {(ebitda_ttm if _np.isfinite(ebitda_ttm) else _np.nan):,.0f} = **{(ev_ebitda if _np.isfinite(ev_ebitda) else _np.nan):,.2f}**"
                )

                # --- compact TTM summary for the JSON payload
                ttm_simple = {
                    ...
                    "components": {
                        ...
                        "CFO (TTM)": None,
                        "CapEx (TTM)": None,
                        "Shares (latest)": (float(shares_latest) if _np.isfinite(shares_latest) else None),
                        "Current Asset (latest)": None,
                        "Current Liability (latest)": None,
                        "Price (latest)": (float(price_latest) if _np.isfinite(price_latest) else None),
                    },
                }

                # --- human-readable annual JSON: [{year: 2021, Revenue: …, Net Profit: …}, …]
                ...
                # 👉 keep the original button label & filename
            ...
            try:
                ...
                if not q4.empty:
                    ...
                    synth = {
                        ...
                        "IncomeTax": _sum_q("Q_Tax"),
                        "DepAmort": _sum_q("Q_DepAmort"),
                        "EBITDA": _sum_q("Q_EBITDA"),

                        # latest snapshot items for ratio denominators
                        ...
                        "SharePrice": _last_q("Q_EndQuarterPrice"),
                        "Dividend": 0.0,  # keep 0 unless you have quarterly DPS
                    }

                    # Prefer a true TTM EPS if available from calculations.compute_ttm
                    ...
                    if ttm_core and ttm_core.get("TTM EPS") is not None:
                        ...
                        # fallback EPS ≈ NetProfit_TTM / Shares_latest
