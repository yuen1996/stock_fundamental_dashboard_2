...
def _num_or_zero(v: object, default: float = 0.0) -> float:
    try:
        f = float(v)
        if pd.isna(f):
            return default
...
def comma_number_input(label, value, key, decimals=0, allow_callback=True, in_form=None):
    """
    Text input with thousands separators.
    Inside st.form pass in_form=True (or allow_callback=False) to avoid on_change.
    Critically: always mirror the parsed float into st.session_state[key]
    so the saver can read it after submit.
    """
    if in_form is not None:
        allow_callback = not in_form  # in_form=True -> no callback
    ...
    if txt_key not in st.session_state:
        try:
            ...
            if pd.isna(base):
                base = 0.0
            st.session_state[txt_key] = f"{base:,.{decimals}f}"
        except Exception:
            st.session_state[txt_key] = f"{0.0:,.{decimals}f}"

    # optional formatter (only when not inside form)
    kwargs = {}
    if allow_callback:
        def _fmt():
            ...
            try:
                ...
                pass
        kwargs["on_change"] = _fmt

    txt = st.text_input(label, value=st.session_state[txt_key], key=txt_key, **kwargs)

    # parse -> mirror into non-_txt key -> return
    try:
        parsed = float(str(txt).replace(",", ""))
    except Exception:
        parsed = 0.0

    st.session_state[key] = parsed   # ⭐️ makes your saver see the value
    return parsed
...
# Minimal schema so new saves never crash
if "Name" not in df.columns: df["Name"] = ""
if "Year" not in df.columns: df["Year"] = pd.Series(dtype="Int64")
...
if stock_name:
    ...
    if "CurrentPrice" in df.columns and df.loc[mask_stock, "CurrentPrice"].notna().any():
        current_price_default = float(df.loc[mask_stock, "CurrentPrice"].dropna().iloc[-1])  # was iloc[0]
    ...
    with colA:
        if st.button("💾 Save stock settings (sync price)", key="save_stock_meta"):
            ...
            for col in ("CurrentPrice", "Price", "Industry", "IndustryBucket", "Name"):
                if col not in df.columns:
                    df[col] = pd.NA

            canon = (stock_name or "").strip()
            name_series = df["Name"].astype("string").str.strip()
            mask_name = name_series.str.casefold() == canon.casefold()

            if not mask_name.any():
                # Seed a minimal ANNUAL row so the price can live somewhere
                seed = {
                    "Name": canon,
                    "Industry": (industry or "").strip(),
                    "IndustryBucket": (industry_bucket or "General").strip(),
                ...
                # normalize stored Name to the typed version
                df.loc[mask_name, "Name"] = canon
                df.loc[mask_name, "CurrentPrice"] = float(cp)
                df.loc[mask_name, "Price"] = float(cp)
                if industry:
                    df.loc[mask_name, "Industry"] = industry.strip()
                if industry_bucket:
                    df.loc[mask_name, "IndustryBucket"] = industry_bucket

            io_helpers.save_data(df)
            st.success("Stock settings saved & synced.")
            _safe_rerun()
...
if stock_name:
        ...
        for i, year in enumerate(years):
            with tab_annual[i]:
                ...
                if st.button("Reset all fields to 0 for this year", key=f"reset_{year}_annual"):
                    for _, key in INCOME_STATEMENT_FIELDS + BALANCE_SHEET_FIELDS + OTHER_DATA_FIELDS + ANNUAL_CF_FIELDS:
                        base = f"{key}_{year}_annual"
                        st.session_state[base] = 0.0
                        st.session_state[base + "_txt"] = f"{0.0:,.0f}" if key != "SharePrice" else f"{0.0:,.4f}"
                    _safe_rerun()
                ...
                for label, key in INCOME_STATEMENT_FIELDS:
                    year_data[key] = comma_number_input(
                        label,
                        value=_num_or_zero(prefill.get(key, 0.0), 0.0),
    ...
    with tabs_top[1]:
        ...
        existing_years = sorted(set(
            df.loc[df["Name"] == stock_name, "Year"].dropna().astype(int).tolist()
        ))
        ...
        with cb:
            sel_quarter = st.selectbox(
                ...
                index=3,
                key=f"q_form_quarter_{stock_name}"
            )

        # Prefill from existing row (if any)
        row_q = df[
            (df["Name"] == stock_name) &
            (df["IsQuarter"] == True) &
            (df["Year"].astype("Int64") == int(sel_year)) &
            (df["Quarter"] == sel_quarter)
        ]
        prefill_q = row_q.iloc[0].to_dict() if not row_q.empty else {}

        # Reset stays OUTSIDE the form so it doesn't submit the form
        if st.button("Reset all fields to 0 for this quarter", key=f"reset_{sel_year}_{sel_quarter}_q"):
            for _, key in QUARTERLY_FIELDS:
                base = f"{key}_{sel_year}_{sel_quarter}_q"
                st.session_state[base] = 0.0
                decs = 4 if key == "Q_EndQuarterPrice" else 0
                st.session_state[base + "_txt"] = f"{0.0:,.{decs}f}"
            _safe_rerun()


        # ---- Everything below is one FORM (no reruns while typing; Enter submits this form only)
        with st.form(f"q_form_{stock_name}_{sel_year}_{sel_quarter}", clear_on_submit=False):
            ...
            for label, key in QUARTERLY_IS_FIELDS:
                ...
                comma_number_input(label, value=val, key=wkey, decimals=0, in_form=True)
            ...
            for label, key in QUARTERLY_OTHER_FIELDS:
                ...
                val = st.session_state.get(wkey, float(prefill_q.get(key, 0.0) or 0.0))
                comma_number_input(
                    label, value=val, key=wkey,
                    decimals=4 if key == "Q_EndQuarterPrice" else 0,
                    in_form=True
                )

            submit_quarter = st.form_submit_button("💾 Save this quarter")


        # Handle save AFTER the form submits (single, atomic upsert)
        if submit_quarter:
            # Build one complete row
            new_row = {
                "Name": stock_name,
                "Industry": (industry or "").strip(),
                "IndustryBucket": (industry_bucket or "General").strip(),
            ...
            for _, k in QUARTERLY_FIELDS:
                v = st.session_state.get(f"{k}_{sel_year}_{sel_quarter}_q", prefill_q.get(k, 0.0))
                try:
                    new_row[k] = float(str(v).replace(",", "")) if v not in (None, "") else None
                except Exception:
                    new_row[k] = None
            ...
            if cond.any():
                df.loc[cond, list(new_row.keys())] = list(new_row.values())
            else:
                df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)

            # Keep industry fields consistent across this stock
            if new_row["Industry"]:
                df.loc[df["Name"] == stock_name, "Industry"] = new_row["Industry"]
            if new_row["IndustryBucket"]:
                df.loc[df["Name"] == stock_name, "IndustryBucket"] = new_row["IndustryBucket"]

            io_helpers.save_data(df)
            st.success(f"Saved {sel_quarter} {sel_year} for {stock_name}.")

            # clear quick-edit buffer for this stock, then rerun
            st.session_state.pop(f"qeb_quarter_{stock_name}_buf", None)
            _safe_rerun()
            
            # 🔥 clear the Quick Edit buffer for this stock so it reloads from df
            state_key = f"qeb_quarter_{stock_name}_buf"
        ...
        # Delete button is OUTSIDE the form (doesn't submit it)
        if st.button("🗑️ Delete this quarter", key=f"delete_{sel_year}_{sel_quarter}_q"):
            ...
            if cond.any():
                ...
                st.session_state.pop(f"qeb_quarter_{stock_name}_buf", None)
                _safe_rerun()
                # clear buffer so the quick editor refreshes
                state_key = f"qeb_quarter_{stock_name}_buf"
                if state_key in st.session_state:
                    del st.session_state[state_key]
            else:
    ...
    for name in sorted(all_rows["Name"].dropna().unique()):
        ...
        with st.expander(name, expanded=False):
            ...
            if "CurrentPrice" in df.columns and df.loc[mask_name, "CurrentPrice"].notna().any():
                cur_default = float(df.loc[mask_name, "CurrentPrice"].dropna().iloc[-1])
