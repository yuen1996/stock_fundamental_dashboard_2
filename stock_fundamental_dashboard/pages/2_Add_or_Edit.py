...
if stock_name:
    ...
    with tabs_top[1]:
        ...
        for label, key in QUARTERLY_IS_FIELDS:
            ...
            q_data[key] = comma_number_input(label, value=st.session_state.get(wkey, default_val), key=wkey, decimals=0)
        ...
        for label, key in QUARTERLY_OTHER_FIELDS:
            wkey = f"{key}_{sel_year}_{sel_quarter}_q"
            default_val = float(prefill_q.get(key, 0.0) or 0.0)
            q_data[key] = comma_number_input(
                label,
                value=st.session_state.get(wkey, default_val),
                key=wkey,
        ...
        with c1:
            if st.button("💾 Save this quarter", key=f"save_{sel_year}_{sel_quarter}_q"):
                ...
                # Add all numeric values captured above
                for _, k in QUARTERLY_FIELDS:
                    new_row[k] = float(q_data.get(k) or 0.0)
                ...
                # Clear Quick Edit (Quarterly) buffer for this stock so new rows appear immediately
        ...
        with c2:
            if st.button("🗑️ Delete this quarter", key=f"delete_{sel_year}_{sel_quarter}_q"):
                ...
                if cond.any():
                    ...
                    # Clear Quick Edit (Quarterly) buffer for this stock so the deletion reflects immediately
                    _buf_key = f"qeb_quarter_{stock_name}_buf"
                    if _buf_key in st.session_state:
                        del st.session_state[_buf_key]
...
# ===== 🧨 Danger zone — Bulk delete stocks (uses current filters) =====
st.markdown("### 🧨 Danger zone: Bulk delete stocks")

names_filtered = sorted(all_rows["Name"].dropna().unique().tolist())
if not names_filtered:
    st.info("No stocks match the current filters above.")
else:
    # Build a small tick-table so you can select which stocks to delete
    _bulk_df = pd.DataFrame({"Delete": [False]*len(names_filtered), "Name": names_filtered})

    # Add Industry & Bucket for context
    _meta = (
        df[df["Name"].isin(names_filtered)]
        .groupby("Name", as_index=False)[["Industry", "IndustryBucket"]]
        .first()
    )
    _bulk_df = _bulk_df.merge(_meta, on="Name", how="left")
    ...
    edited_bulk = st.data_editor(
        ...
        column_config={
            ...
            "IndustryBucket": st.column_config.TextColumn("Bucket", disabled=True),
        },
    )

    col_del1, col_del2 = st.columns([2, 1])
    with col_del1:
        confirm = st.text_input("Type DELETE to confirm bulk deletion", key="bulk_delete_confirm")
    with col_del2:
        if st.button("🗑️ Delete selected stocks", key="bulk_delete_button"):
            if (confirm or "").strip().upper() != "DELETE":
                st.error("Please type DELETE to confirm.")
            else:
                selected = (
                    edited_bulk.loc[edited_bulk["Delete"] == True, "Name"]
                    .dropna().astype(str).unique().tolist()
                )
                if not selected:
                    st.info("No stocks ticked.")
                else:
                    ...
                    # Clear Quick Edit (Quarterly) buffers so UI refreshes cleanly
                    for nm in selected:
                        bkey = f"qeb_quarter_{nm}_buf"
                        if bkey in st.session_state:
                            del st.session_state[bkey]

                    st.warning(
                        f"Deleted {removed_rows} row(s) across {len(selected)} stock(s): "
                        f"{', '.join(selected[:6])}{'…' if len(selected) > 6 else ''}"
                    )
                    _safe_rerun()
