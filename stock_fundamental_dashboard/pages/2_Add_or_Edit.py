...
# Ensure mandatory columns exist (brand-new CSV safety)
for c, default in [
    ("Name", ""),
    ("Industry", ""),
    ("IndustryBucket", ""),
...
if "Year" not in df.columns:
    df["Year"] = pd.Series(dtype="Int64")  # nullable int


# Ensure mandatory columns exist for brand-new files
...
# ---- Annual saver wrapped in a form so it won't trigger while typing elsewhere
with st.form("save_annual_changes_form"):
    submit_annual = st.form_submit_button("💾 Save All Annual Changes")
...
if submit_annual:
    ...
    ind_val = (industry or "").strip()
    bucket_val = (industry_bucket or "General").strip()
    ...
    for year, data in annual_data.items():
        ...
        cond = (
            (df["Name"] == stock_name) &
            (df["Year"].astype("Int64") == y) &
            (df["IsQuarter"] != True)
        )
        if cond.any():
            df.loc[cond, list(row_up.keys())] = list(row_up.values())
        else:
            df = pd.concat([df, pd.DataFrame([row_up])], ignore_index=True)

    # unify industry/bucket across this stock
    if ind_val:
        df.loc[df["Name"] == stock_name, "Industry"] = ind_val
    if bucket_val:
        df.loc[df["Name"] == stock_name, "IndustryBucket"] = bucket_val
    ...
    with tabs_top[1]:
        ...
        existing_years = sorted(
            set(
                df.loc[df["Name"] == stock_name, "Year"]
                .dropna()
                .astype(int)
                .tolist()
            )
        ...
        # Wrap inputs + save in ONE form (prevents half-built saves/reruns)
        ...
        if submit_quarter:
            # Build one complete row from the inputs
            new_row = {
                "Name": stock_name,
            ...
            # Optional: sync industry fields across this stock
            if new_row["Industry"]:
        ...
        # Delete button can stay outside the form
    ...
    for name in sorted(all_rows["Name"].dropna().unique()):
        ...
        with st.expander(name, expanded=False):
            ...
            with tabs[1]:
                ...
                # A small helper to add a ready-to-edit row so you don't have to poke the first blank cell
                col_add, _ = st.columns([1, 3])
                with col_add:
                    if st.button(f"➕ Add empty quarter row for {name}", key=f"add_qrow_{name}"):
                        add = pd.DataFrame([{
                            "Name": name,
                            "Industry": industry_default_this,
                            "IndustryBucket": bucket_default_this,
                            "Year": pd.NA,
                            "Quarter": "—",
                            "IsQuarter": True,
                        }])
                        # ensure all Q_* columns exist in the new row
                        for col in (QUARTERLY_ALLOWED_BASE - {"Name", "Industry", "IndustryBucket", "Year", "Quarter", "IsQuarter"}):
                            add[col] = pd.NA
                        st.session_state[state_key] = pd.concat([st.session_state[state_key], add], ignore_index=True)
