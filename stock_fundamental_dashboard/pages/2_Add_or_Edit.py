...
with colA:
    if st.button("💾 Save stock settings (sync price)", key="save_stock_meta"):
        ...
        # Fill/align IndustryBucket for blanks (new)
        if industry_bucket:
            blank_bucket = df["IndustryBucket"].astype(str).str.strip().isin(["", "None", "nan"])
            df.loc[mask_stock & blank_bucket, "IndustryBucket"] = industry_bucket
