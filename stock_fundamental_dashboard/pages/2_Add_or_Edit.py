    ...
    with tabs_top[1]:
        ...
        with c1:
            if st.button("💾 Save this quarter", key=f"save_{sel_year}_{sel_quarter}_q"):
                ...
                # pick up all numeric quarterly fields
                ...
                # Upsert (update if exists, else append) – do this ONCE (outside the loop above)
                mask = (
                    (df.get("Name") == stock_name) &
                    (df.get("IsQuarter") == True) &
                    (df.get("Year") == int(sel_year)) &
                    (df.get("Quarter") == sel_quarter)
                )
                if mask.any():
                ...
                if "Industry" in df.columns and new_row["Industry"]:
                    df.loc[df["Name"] == stock_name, "Industry"] = new_row["Industry"]
                if "IndustryBucket" in df.columns and new_row["IndustryBucket"]:
