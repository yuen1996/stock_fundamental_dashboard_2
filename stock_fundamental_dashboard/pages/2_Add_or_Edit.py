...
if stock_name:
    ...
    with tabs_top[1]:
        ...
        # ---------- NEW: Multi-quarter chips (like Annual) ----------
        ...
        # Render a subform for each selected quarter
        for token in sel_tokens:
            ...
            if st.button(f"Reset all fields to 0 for {q} {y}", key=f"reset_{y}_{q}_q"):
                for _, key in QUARTERLY_FIELDS:
                    st.session_state[f"{key}_{y}_{q}_q"] = 0.0
                _safe_rerun()
            ...
            for label, key in QUARTERLY_IS_FIELDS:
                ...
                comma_number_input(label, value=st.session_state.get(wkey, default_val), key=wkey, decimals=0)
            ...
            # Optional: per-quarter delete (keeps your old behavior available)
