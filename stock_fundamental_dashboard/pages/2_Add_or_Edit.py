...
# ---- Annual saver wrapped in a form so it doesn't trigger while you're typing elsewhere
...
if submit_annual:
    ...
    # Write/Upsert each selected year
    ...
    # Keep Industry / Bucket consistent across this stock
    if ind_val:
    ...
    with tabs_top[1]:
        ...
        with c1:
            if st.button("💾 Save this quarter", key=f"save_{sel_year}_{sel_quarter}_q"):
                ...
                # gather all quarter fields from session state
                ...
                # make sure all target columns exist before assignment
                for c in new_row.keys():
                    if c not in df.columns:
                        df[c] = pd.NA

                # Upsert ONCE (outside the loop)
                cond = (
                    (df["Name"] == stock_name) &
                    (df["IsQuarter"] == True) &
                ...
                # keep industry fields consistent for this stock
                if new_row["Industry"]:
