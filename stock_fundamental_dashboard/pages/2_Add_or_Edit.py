...
def comma_number_input(label, value, key, decimals=0, allow_callback=True):
    """
    A text-input that shows commas. When allow_callback=False (use this inside st.form),
    it avoids on_change so Streamlit doesn't error.
    """
    txt_key = key + "_txt"

    # Initialize display value once
    ...
    # only attach on_change when we're NOT inside a form
    kwargs = {}
    if allow_callback:
        def _fmt():
