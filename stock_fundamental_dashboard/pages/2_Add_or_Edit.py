        ...
        with st.expander(name, expanded=False):
            ...
            with tabs[0]:
                ...
                if st.button(f"💾 Save Annual for {name}", key=f"qeb_save_a_{name}"):
                    ...
                    # ✅ Ensure all allowed Annual columns (and admin cols) exist so writes to NEW columns persist
                    for col in ANNUAL_ALLOWED_BASE:
                        if col not in df.columns:
                            df[col] = pd.NA
                    for col in ["IndustryBucket", "Industry", "IsQuarter", "Quarter", "Year", "Name"]:
                        if col not in df.columns:
                            df[col] = pd.NA
                    ...
                    for _, er in keep.iterrows():
                        ...
                        if not er.get("RowKey"):  # new row
                            ...
                            # make sure df has every key we’re about to append
                            for k in row_up.keys():
                            ...
                            # ✅ update the IndustryBucket alongside Industry
                            ...
                            for c in keep.columns:
                                if c in ("RowKey", "Delete"):
                                    continue
                                # columns already ensured to exist above, so this will always write
                                if c in ANNUAL_ALLOWED_BASE:
                    ...
                    # ⬇️ unify bucket across ALL rows for this stock using the edited value(s)
                    try:
