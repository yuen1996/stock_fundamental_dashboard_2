...
if stock_name:
    ...
    with colB:
        ...
        confirm_txt = st.text_input(
            ...
            key=f"confirm_delete_{stock_name}"
        )

        if st.button("🗑️ Delete ENTIRE stock (all years & quarters)", key=f"delete_stock_all_{stock_name}"):
            if (confirm_txt or "").strip().upper() != "DELETE":
                ...
                if deleted_rows == 0:
                    ...
                    # Drop every row for this stock
                    df.drop(df[mask].index, inplace=True)

                    # Save changes
                    io_helpers.save_data(df)

                    # Clear Quick Edit (Quarterly) buffer for this stock so UI refreshes cleanly
                    buf_key = f"qeb_quarter_{stock_name}_buf"
                    if buf_key in st.session_state:
                        del st.session_state[buf_key]

                    st.warning(f"Deleted {deleted_rows} row(s) for {stock_name}.")
                    _safe_rerun()
