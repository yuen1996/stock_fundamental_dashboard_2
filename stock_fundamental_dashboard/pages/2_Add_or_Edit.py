...
def comma_number_input(label, value, key, decimals=0, allow_callback=True, in_form=None):
    ...
    if txt_key not in st.session_state:
        try:
            st.session_state[txt_key] = f"{float(value):,.{decimals}f}"
        except Exception:
            st.session_state[txt_key] = str(value)
...
if stock_name:
    ...
    with colA:
        if st.button("💾 Save stock settings (sync price)", key="save_stock_meta"):
            ...
            # robust, normalized match by name (case/space-insensitive)
            canon = (stock_name or "").strip()
            name_series = df["Name"].astype("string").str.strip()
            mask_name = name_series.str.casefold() == canon.casefold()

            if mask_name.any():
                # normalize the stored Name to the canonical spelling the user typed
                ...
                st.warning("No rows found for this stock yet. Add at least one annual/quarter row, then save the price.")
